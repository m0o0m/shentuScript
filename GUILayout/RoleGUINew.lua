...
function RoleGUINew.main()
    ...
    if _GUIHandle ~= 0 then
	    ...
	    _GUIHandle = GUI:ButtonCreate(_Parent,"FashionEquimentPage",1805900076,-17,203)
    if _GUIHandle ~= 0 then
        GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbClick, "RoleGUINew.FashionEquiment")
        GUI:WndSetTextM(_GUIHandle,"时装")
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetTextColorM(_GUIHandle, 4291736418)
        GUI:ButtonSetDownTextPosChange(_GUIHandle, false)
    end
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent,"RoleSkillPage",1805900076,-17,258)
    if _GUIHandle ~= 0 then
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent,"RoleTitlePage",1805900076,-17,313)
    if _GUIHandle ~= 0 then
   
        GUI:WndSetTextM(_GUIHandle,"称号")
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetTextColorM(_GUIHandle, 4291736418)
        GUI:ButtonSetDownTextPosChange(_GUIHandle, false)
    end
...
function RoleGUINew.ShowChildWnd(index)    --show index-page only
	local childWnd = {"RoleEquiment","FashionEquiment","RoleSkill", "RoleRein"}
	for i = 1, #childWnd do
...
function RoleGUINew.RoleWnd(_Handle)
    local _Wnd = GetWindow(RoleGUINew.WndHandle,"RoleEquiment")--微端代码中为RoleEquiment
       ...
       CL:OnRoleWindow()
      for i=1,#emt do 
	  _GUIHandle = GUI:WndFindWindow(_Wnd, emt[i])
      if _GUIHandle ~= 0 then
     	GUI:WndSetVisible(_GUIHandle,false)
    end
	end
		         for i=1,#emp do 
	  _GUIHandle = GUI:WndFindWindow(_Wnd, emp[i])
      if _GUIHandle ~= 0 then
...
function RoleGUINew.FashionEquiment(_Handle)
    local _Wnd = GetWindow(RoleGUINew.WndHandle,"RoleEquiment")--微端代码中为RoleEquiment
	 CL:OnRoleWindow()
         for i=1,#emt do 
	  _GUIHandle = GUI:WndFindWindow(_Wnd, emt[i])
