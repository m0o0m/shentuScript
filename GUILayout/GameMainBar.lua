...
---------------------------------------------------------------------------------
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent, "newMail", 1800500162, 440, -70)	--new邮件
	if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:ButtonCreate(_Parent,"RideHorse",1800500035,196,-16)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnRideBtn")
	...
	_GUIHandle = GUI:ButtonCreate(_Parent,"Setting",1800500027,147,-15)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnSettingBtn")
		GUI:WndSetHint(_GUIHandle,"系统设置")
		GUI:WndSetParam(_GUIHandle, 0)
		GUI:WndSetSizeM(_GUIHandle,40, 40)
	end
	
	
	_GUIHandle = GUI:ButtonCreate(_Parent,"Mail",1800500290,245,-24)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnMailBtn")
		GUI:WndSetHint(_GUIHandle,"邮件")
		GUI:WndSetParam(_GUIHandle, 0)
		GUI:WndSetSizeM(_GUIHandle,40, 40)
	end
	
	
	_GUIHandle = GUI:ImageCreate(_Parent,"Mailimage",1801500188,246,7)
	if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:ButtonCreate(_Parent,"Guild",1800500286,445,-23)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnHangHuiBtn")
	...
	_GUIHandle = GUI:ImageCreate(_Parent,"Familyimage",1801500187,446,8)
	if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:ButtonCreate(_Parent,"Trade",1800500278,396,-27)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnTradeBtn")
	...
	_GUIHandle = GUI:ImageCreate(_Parent,"Tradeimage",1801500185,397,8)
	if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:ButtonCreate(_Parent,"MakeTeam",1800500282,347,-27)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnTeamBtn")
	...
	_GUIHandle = GUI:ImageCreate(_Parent,"Teamimage",1801500186,348,8)
	if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:ButtonCreate(_Parent,"Contact",1800500270,298,-27)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnContactBtn")
	...
	_GUIHandle = GUI:ImageCreate(_Parent,"Friendimage",1801500183,299,8)
	if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:ButtonCreate(_Parent,"Achievement",1800500254,494,-23)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnAchievementBtn")
		GUI:WndSetHint(_GUIHandle,"成就(I)")
		GUI:WndSetParam(_GUIHandle, 0)
		GUI:WndSetSizeM(_GUIHandle,40, 40)
	end
	
	
	_GUIHandle = GUI:ImageCreate(_Parent,"AchieveNum2",0,541,-35)
	if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:ImageCreate(_Parent,"Achievementimage",1801500177,494,8)
	if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:ButtonCreate(_Parent,"QuiteToLogin",1930000080,544,1)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnQuiteToLoginBtn")
		GUI:WndSetHint(_GUIHandle,"退到选择角色界面(Alt+X)")
		GUI:WndSetParam(_GUIHandle, 0)
		GUI:WndSetSizeM(_GUIHandle,33, 20)
	end
	
	
	_GUIHandle = GUI:ButtonCreate(_Parent,"Quite",1930000084,584,1)
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,1002, "GameMainBar.OnQuiteGameBtn")
		GUI:WndSetHint(_GUIHandle,"退出游戏(Alt+Q)")
		GUI:WndSetParam(_GUIHandle, 0)
		GUI:WndSetSizeM(_GUIHandle,33, 20)
	end
    
    --[[
    _GUIHandle = GUI:ButtonCreate(_Parent,"Wing",1800500246,276,-27)
    if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "GameMainBar.OpenWingFun")
        GUI:WndSetHint(_GUIHandle,"翅膀(O)")
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,40, 40)
    end
    
    _GUIHandle = GUI:ImageCreate(_Parent,"Wingimage",1801500179,277,8)
    if _GUIHandle ~= 0 then
        GUI:WndSetEnableM(_GUIHandle, false)
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,46, 20)
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent,"Shenlu",1800500258,227,-27)
    if _GUIHandle ~= 0 then
        GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbClick, "GameMainBar.OpenSLFun")
        GUI:WndSetHint(_GUIHandle,"神炉(L)")
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,48, 44)
    end
	
    
    _GUIHandle = GUI:ImageCreate(_Parent,"Shenluimage",1801500176,228,8)
    if _GUIHandle ~= 0 then
        GUI:WndSetEnableM(_GUIHandle, false)
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,46, 20)
	...
	_GUIHandle = GUI:ImageCreate(_Parent, "SlNum2", 0, 255, -35)-- 神炉数字显示
    
    _GUIHandle = GUI:ButtonCreate(_Parent,"Enhance",1800500242,178,-27)
    if _GUIHandle ~= 0 then
        GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "GameMainBar.OpenEnhanceFun")
        GUI:WndSetHint(_GUIHandle,"锻造(X)")
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,40, 40)
    end
    
    _GUIHandle = GUI:ImageCreate(_Parent,"Enhanceimage",1801500180,179,8)
    if _GUIHandle ~= 0 then
        GUI:WndSetEnableM(_GUIHandle, false)
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,46, 20)
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent,"Crystal",1800500298,129,-27)
    if _GUIHandle ~= 0 then
        GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbClick, "GameMainBar.OnCrystalBtn")
        GUI:WndSetHint(_GUIHandle,"宝石(Y)")
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,48, 44)
    end
    
    _GUIHandle = GUI:ImageCreate(_Parent,"Crystalimage",1801500190,130,8)
    if _GUIHandle ~= 0 then
        GUI:WndSetEnableM(_GUIHandle, false)
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,46, 20)
	...
	-- _GUIHandle = GUI:ImageCreate(_Parent, "redPacketBtn", 1891410099, -480, -170)
	-- if _GUIHandle ~= 0 then 
		-- GUIImageSetAnimateEnable(_GUIHandle, true, 150)
		-- GUI:WndSetSizeM(_GUIHandle,64, 64)
		-- GUI:WndSetCanRevMsg(_GUIHandle,true)
		-- GUI:WndRegistScript(_GUIHandle, RDWndBaseCL_mouse_lbClick, "GameMainBar.OnRedPacketClicked")
		-- GUI:WndMoveToParentTop(_GUIHandle)
	-- end
	
    _GUIHandle = GUI:ButtonCreate(0, "RMBBtn",1803700007,0, 0)
    if _GUIHandle ~= 0 then
        GUI:WndSetTextM(_GUIHandle,"玩家寄售")
        GUI:WndSetHint(_GUIHandle,"寄售系统")
        GUI:WndSetParam(_GUIHandle, 0)
		GUI:WndRegistScript(_GUIHandle, RDWndBaseCL_mouse_lbUp, "GameMainBar.OnRMBBtn")
		...
		GUI:ButtonSetStateTextColor(_GUIHandle, 0, mkcolor("GOLD"));
		GUI:ButtonSetStateTextColor(_GUIHandle, 1, mkcolor("GOLD"));
		GUI:ButtonSetStateTextColor(_GUIHandle, 2, mkcolor("GOLD"));
    end
	------------------------------------------------------------------
...
function GameMainBar.UIInit(_Parent)
	...
	CL:HotKeyRegister("Y", "打开宝石窗口", "GameMainBar.OnCrystalBtn");  --注册宝石热键Y
	CL:HotKeyRegister("X", "打开锻造窗口", "GameMainBar.OpenEnhanceFun"); --注册锻造热键X
	CL:HotKeyRegister("L", "打开神炉窗口", "GameMainBar.OpenSLFun"); --注册神炉热键L
	CL:HotKeyRegister("O", "打开翅膀窗口", "GameMainBar.OpenWingFun"); --注册翅膀热键O
	...
	str = str .. "<wnd id=消息框 x=250 y=-100 w=390 h=72 revmsg=0 >";
	str = str .. "<image id=背景 x=0 y=0 w=390 h=72 image=1903700039 fitsize=1 />";
	str = str .. "<richedit id=内容 x=0 y=0 w=386 h=20 />";
	str = str .. "</wnd></form>";
	GenFormByString(str);
	local handle = GetWindow(nil, "GameMainInterface,GameMainBar,消息框");
	RichEditSetTotalLine(handle, "内容", 5);
	ImageSetAlpha(handle, "背景", 50);
	-- UI:Lua_AddDelayTask("GameMainBar.UpdataSize()", 0, 1);
	--------------------------------------------------------------------------
end
...
function GameMainBar.AddRedPacketBtnEffect()
	_GUIHandle = GetWindow(GameMainBar.WndHandle, "redPacketBtnMagic")
	if _GUIHandle ~= 0 then 
...
function GameMainBar.UpdataScreenSize()
    ...
    local _RMBBtn = GetWindow(0, "RMBBtn");
    if _RMBBtn ~= 0 then
        GUI:WndSetPosM(_RMBBtn, newX-90, newY/2-50)
    end
...
GameMainBar.UpdataSize = function()
	...
	local handle1 = GetWindow(handle, "消息框");
...
function GameMainBar.OnRMBBtn()

	local handle = GetWindow(0, "TradeWnd");
	if handle ~= 0 then 
		GUI:WndClose(handle);
	else
		UI:Lua_OpenWindow(0, "TradeWnd.lua");
	end
end
