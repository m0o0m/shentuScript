...
function tipItemsLayoutInitialize(ihandle, ehandle, equip)
	...
	if equip and qiangh_num~= 0  then
...
function tipItemsLayoutTitle(ihandle, ehandle, equip)
	...
	ret = ret..tipSet2(15,17)..tipText(name, color)
	...
	if BigIcon ~= 0 and BigIcon ~= nil then
		ret = ret..tipSet2(25,64)..tipImage(1803100002,58,58)..tipSet2(25,64)..tipImage(BigIcon, 58, 58)
	end	
	...
	if ehandle ~= 0 and equip then
		...
		if refine > 0 then
			if refine > 10 then
				ret = ret .. tipSet2(48, 63) .. tipText("+"..refine, "#ffcc00")
			else
				ret = ret .. tipSet2(60, 63) .. tipText("+"..refine, "#ffcc00")
			end
		end
		--蕴魂
		...
		if zhul_lv > 0 then
			if zhul_lv > 10 then
				ret = ret .. tipSet2(48, 99) ..tipText("+"..zhul_lv, "#00ccff")
			else
				ret = ret .. tipSet2(60, 99) ..tipText("+"..zhul_lv, "#00ccff")
			end
		end
	end
...
function tipItemsLayoutBind(ihandle, ehandle, equip)
	...
	if bound then 
		...
		if equip then
			if qiangh_num > 0 then
				ret = ret..tipSet2(300, 17)..tipText(state, color)
			else
				ret = ret..tipSet2(200, 17)..tipText(state, color)
			end	
		else
			ret = ret..tipSet2(200, 17)..tipText(state, color)
		end
	...
	if equip then
		...
		if LuaRet ~= 0 then
			...
			if max_use ~= 0 and GetItemGUIDataItemPos() ~= SKILLBAR_POS then
				...
				ret = ret..tipSet2(240, 68)..tipText("次数：",4294573201)	..tipText(cur_use.."/"..max_use.."\n")
			end
		end
	end
...
function tipItemsLayoutFumoshi(ihandle, ehandle, equip)
	...
	if not equip and ehandle ~= 0 and GetTipParam("TYPE") == 4 and GetTipParam("SUBTYPE") == 20 then
		...
		if str == "" then
			ret = tipSet2(40)..tipText("无附魔属性\n", 4289309097)
		else
			ret = tipSet2(40)..tipText("附魔属性\n", 4294953984)..str
		end
	end
...
function tipItemsLayoutAttackDefense(ihandle, ehandle, equip)
	...
	if vmin ~= 0 or vmax ~= 0 or ext_table[10] or att_table[10] then
		...
		ret = ret..tipSet2(40, 0)..tipText("物理攻击："..vmin.." - "..vmax, __color[thigh])
		--[[if ext_table[10] ~= nil then
	...
	if vmin ~= 0 or vmax ~= 0 or ext_table[12] or att_table[12] then
		...
		ret = ret..tipSet2(40, 0)..tipText("魔法攻击："..vmin.." - "..vmax, __color[thigh])
	...
	if vmin ~= 0 or vmax ~= 0 or ext_table[14] or att_table[14] then
		...
		ret = ret..tipSet2(40, 0)..tipText("道术攻击："..vmin.." - "..vmax, __color[thigh])
		--[[if ext_table[15] ~= nil and ext_table[14] ~= nil  then
	...
	if vmin ~= 0 or vmax ~= 0 then
		...
		ret = ret..tipSet2(40, 0)..tipText("物理防御："..vmin.." - "..vmax, __color[thigh])
		--[[if ext_table[7] ~= nil and ext_table[6] ~= nil  then
	...
	if vmin ~= 0 or vmax ~= 0 then
		...
		ret = ret..tipSet2(40, 0)..tipText("魔法防御："..vmin.." - "..vmax, __color[thigh])
		--[[if ext_table[9] ~= nil and ext_table[8] ~= nil then
...
function tipItemsLayoutOtherProperty(ihandle, ehandle, equip, notnull)
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("生命："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("魔法："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("背包负重："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("穿戴负重："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("腕力："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("命中："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("闪避："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("魔法闪避："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if not ehandle and equip and subType == 13 then 
		...
		if hunzhu_burstharm[name] ~= nil then 
			ret = ret..tipSet2(40, 0)..tipText("暴击伤害："..hunzhu_burstharm[name].."\n", 4294901760)
		end
	...
	if val ~= 0 and val <= 10 and val >= -10 then
		...
		if val > 0 then ret = ret..tipSet2(40, 0)..tipText("幸运：+"..val.."\n", __goodcol[val])
		else val = -val; ret = ret..tipSet2(40, 0)..tipText("诅咒：+"..val.."\n", __badcol[val]) end
	end
	...
	if GetTipParam("ITEMIDENTIFIER") == "气血石" or GetTipParam("ITEMIDENTIFIER") == "火龙之心" then
		...
		if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("强度："..__pading[val > 0]..val.."\n", __color[thigh]) end
	end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("移动速度："..__pading[val > 0]..val.."%\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("攻击速度："..__pading[val > 0]..val.."%\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("中毒回复："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("生命回复："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipSet2(40, 0)..tipText("魔法回复："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	--[[if ehandle ~= 0 and equip then
		...
		if qiangh_num > 0 and refine < 12 then
			...
			ret = ret .. tipText("\n")	
			if subType == 1 or subType == 13 or subType == 9 or subType == 8 or subType == 3 then	
				if level >= 100 then
						for i = 1 , 3 do 
	...
	if notnull or ret ~= "" then ret = ret..tipLine()..tipSet(nil, 8) end; return ret
end
		...
		if item_keyname ~= "" then
			...
			for i = 1 , 4 do 
				...
				elseif item_keyname == "2" then
					ret = ret .. tipSet(5,3) .. tipImage(1802900031,19,19) .. tipSet(10,-3) .. tipText("强化".. i*3 .."级可镶嵌", "#BFCDDB")
				elseif item_keyname == "1" then
					ret = ret .. tipSet(5,3) .. tipImage(1800700057,19,19) .. tipSet(10,-3) .. tipText("可钻孔", "#BFCDDB")
				else
					local stoneType = string.match(item_keyname,"级".."(.-)".."宝")
						...
						if propertyType then
							...
							if a == 40 then
								ret = ret .. tipSet2(205) .. tipText(propertyType, "#7030A0")
								blood = CL:GetItemEntityCustomVarInt(Entity, "_dv"..a)
								ret = ret .. tipSet2(300) .. tipText(blood, "#7030A0")
							else
								ret = ret .. tipSet2(205) .. tipText(propertyType, "#7030A0")
								mi = CL:GetItemEntityCustomVarInt(Entity, "_dv"..a)
								ma = CL:GetItemEntityCustomVarInt(Entity, "_dv"..a + 1)
								ret = ret .. tipSet2(300) .. tipText(mi.." - "..ma, "#7030A0")
							end
						end
					else
						ret = ret .. tipSet(5,0) .. tipSet(10,0) .. tipText("服务端存了不合法的keyname")
					end
				end
				ret = ret .. tipText("\n")..tipSet(40,3)
			end
...
function tipItemsLayoutSuit(ihandle, ehandle, equip)
	...
	if UI:Lua_GetItemTemplatePropByHandle(ihandle, ITEM_PROP_SUIT) and LuaRet ~= 0 then
		local suitid = LuaRet
		local __color = {[true] = 4281645312, [false] = 4287861651}
		...
		ret = ret..tipSet2(40)..tipText(name.."("..tcount.."/"..#suitlist..")\n", __color[true])
		--显示套装条目--
		for suit,id in pairs(suitlist) do
            if UI:Lua_GetItemTemplatePropByID(id, ITEM_PROP_NAME) then
				ret = ret..tipSet(52)..tipText(LuaRet.."\n", __color[tsuit[id] ~= nil])
			end
        end
		ret = ret..tipSet2(40)..tipText("套装属性\n", __color[true])
		--显示套装属性--
		...
		for i = 1,#suit_prop_size do
			if suit_prop_size[i][2] > 0 and suit_prop_value[i][2] > 0 and UI:Lua_GetAttrProp(suit_prop_attr[i][2],ATT_PROP_CHINANAME) then
				...
				ret = ret..tipSet(52)..tipText(suit_prop_size[i][2].."/"..#suitlist.." "..GetAttNameValueStr(LuaRet, suit_prop_value[i][2], "提升").."\n", __color[has_attr])
			end
		end
	end
	--if ret ~= "" then ret = tipSet(nil, -8)..tipLine()..ret..tipSet(nil, 8) end; return ret
	if ret ~= "" then ret = tipSet(nil, -8)..ret..tipLine()..tipSet(nil, 8) end; return ret
end
...
function tipItemsLayoutPrice(ihandle, ehandle, equip)
	...
	if itempos == GAMESTALL_POS or itempos == VISITSTALL_POS then
		...
		if __stall[price_type] ~= nil then
			...
			ret = ret..tipSet2(40)..tipText("售价：".._FormatPrice..__stall[price_type].name.."\n", _StallColor)
		end
	end
