...
CreateWnd = function(_Handle, _Info, _ID)
	...
	if not UI:Lua_CreateWnd(_ID, Temporary) then return 0 end
	if not UI:Lua_WndLoadLayOut(_ID) then return 0 end
	return Temporary
end
...
CreateWndFromFile = function(_Handle, _Info, _ID, _FileName)
	if not WndAttach(_Handle, _Info) then return 0 end
	if not UI:Lua_CreateWnd(_ID, Temporary) then return 0 end
	if not UI:Lua_WndLoadLayOut(_FileName) then return 0 end
	return Temporary
end
...
WndGetEnableM = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return false end
	if UI:Lua_WndGetEnableM() then return LuaRet end
	return false
...
WndGetParam = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_WndGetParam() then return LuaRet end
	return 0
end
...
WndGetProperty = function(_Handle, _Info, _Key)
	if not WndAttach(_Handle, _Info) then return "" end
	if UI:Lua_WndGetProperty(_Key) then return LuaRet end
	return ""
end
...
WndGetScreenPos = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0,0 end
	if UI:Lua_WndGetScreenPos() then return LuaRet[1], LuaRet[2] end
	return 0, 0
end
...
WndAddTimerBrief = function(_Handle, _Info, _Elapse, _ScriptHandle)
	if not WndAttach(_Handle, _Info) then return false end
	if not UI:Lua_WndAddTimerBrief(_Elapse, _ScriptHandle) then return false end
	return LuaRet
end
...
WndGetItemGUIDataHandle = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_WndGetItemGUIDataHandle() then return LuaRet end
	return 0
end
...
WndGetCurItemWindow = function()
	if UI:Lua_WndGetCurItemWindow() then return LuaRet end
	return 0
end
...
ButtonGetIsActivePageBtn = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return false end
	if not UI:Lua_ButtonGetIsActivePageBtn() then return false end
	return LuaRet
end
...
EditGetBNumber = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return false end
	if UI:Lua_EditGetBNumber() then return LuaRet end
	return false
end
...
EditGetTextM = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return "" end
	if UI:Lua_EditGetTextM() then return LuaRet end
	return ""
...
EditGetInt = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_EditGetInt() then return LuaRet end
	return 0
end
...
EditGetMaxCharNum = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_EditGetMaxCharNum() then return LuaRet end
	return 0
end
...
ImageGetImageID = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_ImageGetImageID() then return LuaRet end
	return 0
end
...
ImageGetColor = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_ImageGetColor() then return LuaRet end
	return 0
end
...
ImageGetAlpha = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_ImageGetAlpha() then return LuaRet end
	return 0
end
...
RichEditGetSelectClickString = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return "" end
	if UI:Lua_RichEditGetSelectClickString() then return LuaRet end
	return ""
end
...
RichEditGetTextItemCount = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_RichEditGetTextItemCount() then return LuaRet end
	return 0
end
...
RDItemCtrlGetGUIData = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_RDItemCtrlGetGUIData() then return LuaRet end
	return 0
end

RDItemCtrlGetGUIDataPropByType = function(_Handle, _Info, _PropType)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_RDItemCtrlGetGUIDataPropByType(_PropType) then return LuaRet end
	return 0
end
...
RDItemCtrlSetGUIDataPropByItemID = function(_Handle, _Info, _ItemID, _Count, _IsBind)
	...
	if not UI:Lua_GetItemTemplateHandleByID(_ItemID) then return false end
	local _ItemHandle = LuaRet
	if not UI:Lua_GetItemTemplatePropByHandle(_ItemHandle, ITEM_PROP_ICON) then return false end
...
RDItemCtrlSetGUIDataPropByKeyName = function(_Handle, _Info, _KeyName, _Count, _IsBind)
	if not UI:Lua_GetItemTemplateHandleByKeyName(_KeyName) then return false end
	if not UI:Lua_GetItemTemplatePropByHandle(LuaRet, ITEM_PROP_ID) then return false end
...
RDItemCtrlSetGUIDataPropByGUID = function(_Handle, _Info, _ItemGUID)
	...
	--物品图片ID
	if not UI:Lua_GetItemTemplatePropByHandle(_Template, ITEM_PROP_ICON) then return false end
	if not RDItemCtrlSetGUIDataPropByType(_Handle, _Info, ITEMGUIDATA_IMAGEID, LuaRet) then return false end
	--物品数量
...
RDItemCtrlFillGUIData = function(_Handle, _Info, _GUIDataHandle)
	if UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ITEMGUID) then
		if not RDItemCtrlSetGUIDataPropByType(_Handle, _Info, ITEMGUIDATA_ITEMGUID, LuaRet) then return false end
	end
	...
	if UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_INVALIDATE) then
		if not RDItemCtrlSetGUIDataPropByType(_Handle, _Info, ITEMGUIDATA_INVALIDATE, LuaRet) then return false end
	end
	return true
end
...
ComboBoxGetText = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return "" end
	if UI:Lua_ComboBoxGetText() then return LuaRet end
	return ""
end
...
ComboBoxGetCurSelect = function(_Handle, _Info)
	...
	if UI:Lua_ComboBoxGetCurSelect() then return LuaRet end
	return -1
end

ComboBoxGetCurSelectID = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_ComboBoxGetCurSelectID() then return LuaRet end
	return 0
end
...
ComboBoxGetEdit = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if not UI:Lua_ComboBoxGetEdit() then return 0 end
	return LuaRet
end
...
CheckGetCheck = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return false end
	if not UI:Lua_CheckGetCheck() then return false end
	return LuaRet
end
...
MenuCreate = function(_Handle, _Info, _WndId)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_MenuCreate(_WndId) then return LuaRet end
	return 0
end

MenuGetMenu = function(_Handle, _Info)
	if not WndAttach(_Handle, _Info) then return 0 end
	if UI:Lua_MenuGetMenu() then return LuaRet end
	return 0
end
