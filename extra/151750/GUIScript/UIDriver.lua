...
function UIDriver_Jiaoben_OnShowForm()
	dbg("UIDriver_Jiaoben_OnShowForm")
	if LuaParam[1] ~= "脚本表单" then return end
	dbg("UIDriver_Jiaoben_OnShowForm.....1")
	local str = LuaParam[2]

	--@zhangxin 有时候直接发过来两个函数 分割符是 ;
	local strTab = strsplit(str, ";")
	for i = 1, #strTab do
		if(strTab[i] ~= "") then
			dbg("i = " .. i .. "  v = " ..strTab[i])
			local func = loadstring(strTab[i])
			if func ~= nil then
				func()
				dbg("UIDriver_Jiaoben_OnShowForm.....2")
			end
		end
	end
end
...
function UIDriver_Formfile_OnShowForm()
	dbg("UIDriver_Formfile_OnShowForm")
	if LuaParam[1] ~= "form文件表单" then
		dbg("UIDriver_Formfile_OnShowForm.....1")
		return
	end
	dbg("UIDriver_Formfile_OnShowForm.....2")
	local str = LuaParam[2]
	UI:Lua_GenForm(str)
end
