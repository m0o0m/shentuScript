...
function ConvCrystal.main()
    ...
    if _GUIHandle ~= 0 then
        GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "ConvCrystal.clean")
    ...
    _GUIHandle = GUI:EditCreate(_Parent,"gold",260,269,0,0)
    if _GUIHandle ~= 0 then
	...
	_GUIHandle = GUI:EditCreate(_Parent, "msg", 88, 222, 0, 0)
	if _GUIHandle ~= 0 then 	
...
function ConvCrystal.put(_Handle, GUIData, DragIn, guid, ItemId, KeyNamee, ItemPos, IsBound, Count)
	...
	if CL:GetItemEntityPropByGUID(guid,ITEM_PROP_TYPE) then
		...
		if chief ==4 and CL:GetItemEntityPropByGUID(guid,ITEM_PROP_SUBTYPE) then
