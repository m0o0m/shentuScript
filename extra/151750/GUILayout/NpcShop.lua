...
function NpcShop.main()
	...
	_GUIHandle = GUI:EditCreate(_Parent, "GlodEdit", offset_X + 40 , offset_Y - 27, 200, 24)
	if _GUIHandle ~= 0 then
		GUI:WndSetEnableM(_GUIHandle, false)
	...
	_GUIHandle = GUI:ImageCreate(_Parent,"Icon_BindGold", 1852600016, offset_X + 150, offset_Y - 30)
	if _GUIHandle ~= 0 then
		GUI:WndSetSizeM(_GUIHandle, 37, 25)
		GUI:WndSetEnableM(_GUIHandle, false)
	end
	_GUIHandle = GUI:EditCreate(_Parent, "BindGlodEdit", offset_X + 190 , offset_Y - 27, 200, 24)
	if _GUIHandle ~= 0 then
		GUI:WndSetEnableM(_GUIHandle, false)
		--GUI:EditSetTextM(_GUIHandle, "1111111111")
		GUI:EditSetTextColor(_GUIHandle, color.text1)
	...
	if _GUIHandle ~= 0 then
		GUI:WndRegistScript(_GUIHandle, RDWndBaseCL_mouse_lbUp, "NpcShop.Close")
	...
	if 0 ~=_GUIHandle then
		...
		NpcShop.hItemList = _GUIHandle
		local intervalY = 110
		for i = 1, 40 do
				local index = i - 1
				_GUIHandle = GUI:ButtonCreate(NpcShop.hItemList, "ItemBtn"..tostring(index), 1850500012, 0, 10 + intervalY*(i - 1))
				if _GUIHandle ~= 0 then
					-- GUI:WndRegistScript(_GUIHandle, RDWndBaseCL_mouse_lbUp, "NpcShop.itemClicked")
					GUI:WndSetSizeM(_GUIHandle, 432, 118)
					-- GUI:WndSetParam(_GUIHandle, index)
					GUI:WndSetVisible(_GUIHandle, false)
				end

				_GUIHandle = GUI:EditCreate(NpcShop.hItemList, "Name"..tostring(index), 160, 26 + 10 + intervalY*(i - 1), 150, 24)
				if _GUIHandle ~= 0 then
				...
				_GUIHandle = GUI:EditCreate(NpcShop.hItemList, "Level"..tostring(index), 320, 26 + 10 + intervalY*(i - 1), 60, 24)
				if _GUIHandle ~= 0 then
					GUI:WndSetEnableM(_GUIHandle, false)
				...
				_GUIHandle = GUI:EditCreate(NpcShop.hItemList, "Price"..tostring(index),  210, 70 + 10 + intervalY*(i - 1), 200, 24)
				if _GUIHandle ~= 0 then
					GUI:WndSetEnableM(_GUIHandle, false)
					GUI:WndSetVisible(_GUIHandle, false)
				end
				...
				_GUIHandle = GUI:ItemCtrlCreate(NpcShop.hItemList, "Item"..tostring(index), 1850400004, 36, 26 + 10 + intervalY*(i - 1), 66, 66)
				if _GUIHandle ~= 0 then
					--RDItemCtrlSetGUIDataPropByItemID(_GUIHandle, nil, 10052, 1, false)
					GUI:WndSetVisible(_GUIHandle, false)
				end

				_GUIHandle = GUI:ImageCreate(NpcShop.hItemList,"lineImg"..tostring(index), 1850500022, 115, 56 + 10 + intervalY*(i - 1))
				if _GUIHandle ~= 0 then
				...
				_GUIHandle = GUI:EditCreate(NpcShop.hItemList, "JBPrice"..tostring(index), 160, 70 + 10 + intervalY*(i - 1), 200, 24)
				if _GUIHandle ~= 0 then
				...
				_GUIHandle = GUI:ImageCreate(NpcShop.hItemList,"DiscountImg"..tostring(index), 1852600015, 7, 0 + intervalY*(i - 1) + 19)
				if _GUIHandle ~= 0 then
					GUI:WndSetEnableM(_GUIHandle, false)
					GUI:WndSetVisible(_GUIHandle, false)
				end
	...
	if _GUIHandle ~= 0 then
		-- GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "NpcShop.subtract")
	end

	_GUIHandle = GUI:ButtonCreate(_Parent,"increaseBtn",1851000016, offset_X + 822, offset_Y + 361)
	if _GUIHandle ~= 0 then
		-- GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "NpcShop.plus")
	end
	...
	if _GUIHandle ~= 0 then
		...
		-- GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "NpcShop.SlideEvent") -- trigger
		-- GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_move, "NpcShop.SlideEvent") -- trigger
	end

	_GUIHandle = GUI:ButtonCreate(_Parent,"buyBindGold_Btn", 1852600003, offset_X + 747, offset_Y + 457)
	if _GUIHandle ~= 0 then
		GUI:WndSetSizeM(_GUIHandle, 157, 47)
		GUI:WndSetTextM(_GUIHandle, "绑定金币购买")
		GUI:WndSetTextColorM(_GUIHandle, color.text1)
		--GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "NpcShop.bindGold_buy")
	end

	_GUIHandle = GUI:ButtonCreate(_Parent,"buyGold_Btn", 1850000307, offset_X + 917, offset_Y + 457)
	if _GUIHandle ~= 0 then
		GUI:WndSetSizeM(_GUIHandle, 107, 47)
		--GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "NpcShop.gold_buy")
		GUI:WndSetTextM(_GUIHandle, "金币购买")
		GUI:WndSetTextColorM(_GUIHandle, color.text1)
	end
	...
	NpcShop.UIInit(_Parent)
end

--UI Logic Code Start
function NpcShop.UIInit(_Handle)
	if 0 ~= GameMainBar.WndHandle then
		GUI:WndSetVisible(GameMainBar.WndHandle, false)
	end

	NpcShop.WndHandle = _Handle
	local _GUIHandle = GUI:WndFindWindow(NpcShop.WndHandle, "totalPrice")
	if 0 ~= _GUIHandle then
		--GUI:EditSetTextM(_GUIHandle, "9999999")
	end
end

-- function NpcShop.itemClicked()
-- end

-- function NpcShop.SlideEvent(_Handle)
-- 	local _GUIHandle = GUI:WndFindWindow(NpcShop.WndHandle, "amount")
-- 	if _GUIHandle then
...
-- 		-- _GUIHandle = GUI:WndFindWindow(NpcShop.WndHandle, "totalPrice")
-- 		-- if 0 ~= _GUIHandle then
...
-- 		-- end
-- 		_GUIHandle = GUI:WndFindWindow(NpcShop.WndHandle, "slide")
-- 		if 0 ~= _GUIHandle then
...
-- 	end
-- end
function NpcShop.OpenShopWnd()
	GUI:WndClose(NpcSell.WndHandle)
	GUI:WndSetVisible(NpcS.WndHandle, true)
end

function NpcShop.OpenSellWnd()
end


function NpcShop.Close()
	GUI:WndClose(NpcShop.WndHandle)
