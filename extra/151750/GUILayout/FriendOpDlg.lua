...
function FriendOpDlg.main()
    ...
    if _GUIHandle ~= 0 then
        GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "FriendOpDlg.Close")
    end
    

    FriendOpDlg.UIInit(_Parent)
end

--UI Logic Code Start
if not FriendOpDlg.localString then FriendOpDlg.localString = {} end
local l = FriendOpDlg.localString
l.addFriendBtn = "添加好友"
...
function FriendOpDlg.UIInit(_Handle)
...
function FriendOpDlg.Close(_Handle)
    if _Handle ~= 0 then 
