...
function ForgeWnd4:main()
    ...
    _Parent = GUI:WndCreateWnd(_Parent,"ForgeWnd4",0,0,0)
    if _Parent ~= 0 then
    end
...
function ForgeWnd4:UIInit(_GUIHandle)
	...
	for i = 1, 3 do
		local handle = GUI:WndFindChildM(_GUIHandle,"button" .. i)
		if handle ~= 0 then
			GUI:WndSetTextArrangeType(handle,0,2,0,0);
	...
	UI:Lua_OpenWindow(_GUIHandle,"SmeltWnd.lua")

	
	-- ForgeWnd4:CreateYunHunList()
end
