...
function SettingProperty.UIInit(_GUIHandle)
	...
	local _percentage = GUI:WndFindChildM(_GUIHandle,"percentage")
	if _percentage ~= 0 then
		GUI:CheckBoxSetAutoChange(_percentage,true)
	end
	...
	for i = 1, 4 do
		for j = 1, 2 do
			...
			local posX = (j - 1) * 278
			local posY = (i - 1) * 50
			local _check = GUI:WndFindChildM(_GUIHandle,"check"..itemIdx)
			if _check ~= 0 then
				GUI:CheckBoxSetAutoChange(_check,true)
			end

			local _ComboBox = GUI:WndFindChildM(_GUIHandle,"ComboBox"..itemIdx)
			if _ComboBox ~= 0 then
				GUI:WndSetParam(_ComboBox, itemIdx)
				GUI:ComboBoxClear(_ComboBox)
				GUI:ComboBoxSetScrollBarImage(_ComboBox, 0, 0, 0, 0)
	...
	for j = 9, 10 do		
		local posX = (a - 1) * 287
		local _check = GUI:WndFindChildM(_GUIHandle,"check"..j)
		if _check ~= 0 then
			GUI:CheckBoxSetAutoChange(_check,true)
		end
		
		local _ComboBox = GUI:WndFindChildM(_GUIHandle,"ComboBox"..j)
		if _ComboBox ~= 0 then
			GUI:WndSetParam(_ComboBox, itemIdx)
			GUI:ComboBoxClear(_ComboBox)
			GUI:ComboBoxSetScrollBarImage(_ComboBox, 0, 0, 0, 0)
	...
	if _Istrue then
		if CL:GetProtectArr(2005) then
			SettingProperty.hp = getTable(LuaRet)	
		end
	else
	...
	if CL:GetProtectArr(2006) then --
		SettingProperty.check1 = getTable(LuaRet)	
	end
	
	if CL:GetProtectArr(2003) then
		SettingProperty.daley = getTable(LuaRet)	
	end
	
	if CL:GetProtectArr(2002) then
		SettingProperty.item = getTable(LuaRet)	
	end
	
	if CL:GetProtectArr(2007) then
		SettingProperty.selectid = getTable(LuaRet)	
	end

	for i = 1 , #SettingProperty.item do 
		local tab = getTable(SettingProperty.item[i])
		...
		for j = 1 , #tab do 
			...
			if CL:GetItemTemplatePropByID(tab[j],1) then
				local name = LuaRet								
				GUI:ComboBoxAddString(_handle, name, j-1, 0)				
			end
		end
		EditSetInt(_hp,nil,SettingProperty.hp[i])
	
		GUI:ComboBoxSetCurSelect(_handle, SettingProperty.selectid[i])

		GUI:CheckBoxSetCheck(_check,SettingProperty.check1[i])
	end
end
function SettingProperty.han_use(_Handle)
	local _Istrue = GUI:CheckBoxGetCheck(_Handle)
	CL:ChangeBasicSetting(2001,_Istrue)
	if _Istrue then
		if CL:GetProtectArr(2005) then
			SettingProperty.hp = getTable(LuaRet)	
		end
...
function SettingProperty.check(_Handle)
	...
	if CL:SetProtectIndex(2006,index,tostring(Is_true)) then
	end
end	

function SettingProperty.EditChange(_Handle)
	local index = GUI:WndGetParam(_Handle)
	local str = GUI:EditGetInt(_Handle)
	local _percent = GetWindow(SettingProperty.WndHandle,"percentage",nil)
	local Is_true = GUI:CheckBoxGetCheck(_percent)
	...
	if Is_true then
		...
		if CL:SetProtectIndex(2005,index,tostring(str)) then
		end
	else
		if CL:SetProtectIndex(2004,index,tostring(str)) then
		end
	end
...
function SettingProperty.EditChange1(_Handle)
	...
	if CL:SetProtectIndex(2003,index,tostring(str)) then
	end
...
function SettingProperty.OnStrChange(_Handle)
	...
	if CL:SetProtectIndex(2007,i,index) then
