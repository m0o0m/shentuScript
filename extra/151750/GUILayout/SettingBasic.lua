...
function SettingBasic.UIInit(_GUIHandle)
	...
	local _PlayerName = GUI:WndFindChildM(_GUIHandle,"ShowPlayerName")
	if _PlayerName ~= 0 then
		GUI:CheckBoxSetAutoChange(_PlayerName,true)
		
	end
	
	local _TPlayerName = GUI:WndFindChildM(_GUIHandle,"TuPlayerName")
	if _TPlayerName ~= 0 then
		GUI:CheckBoxSetAutoChange(_TPlayerName,true)
		
	end
	
	local _NpcName = GUI:WndFindChildM(_GUIHandle,"TuNpcName")
	if _NpcName ~= 0 then
		GUI:CheckBoxSetAutoChange(_NpcName,true)
		
	end
	
	local _Start_msg = GUI:WndFindChildM(_GUIHandle,"Start_msg")
	if _Start_msg ~= 0 then
		GUI:CheckBoxSetAutoChange(_Start_msg,true)
	end
	
	local _NoShift = GUI:WndFindChildM(_GUIHandle,"NoShift")
	if _NoShift ~= 0 then
		GUI:CheckBoxSetAutoChange(_NoShift,true)
		
	end
	
	local _ShowHName = GUI:WndFindChildM(_GUIHandle,"ShowHName")
	if _ShowHName ~= 0 then
		GUI:CheckBoxSetAutoChange(_ShowHName,true)
	end
	
	local _ShowOther = GUI:WndFindChildM(_GUIHandle,"ShowOther")
	if _ShowOther ~= 0 then
		GUI:CheckBoxSetAutoChange(_ShowOther,true)
	end
	
	local _ShowSysMsg = GUI:WndFindChildM(_GUIHandle,"ShowSysMsg")
	if _ShowSysMsg ~= 0 then
		GUI:CheckBoxSetAutoChange(_ShowSysMsg,true)
	end
	
	local _ChixuJ = GUI:WndFindChildM(_GUIHandle,"ChixuJ")
	if _ChixuJ ~= 0 then
		GUI:CheckBoxSetAutoChange(_ChixuJ,true)
	end
	
	local _ShanSItem = GUI:WndFindChildM(_GUIHandle,"ShanSItem")
	if _ShanSItem ~= 0 then
		GUI:CheckBoxSetAutoChange(_ShanSItem,true)
	end
	
	local _Num_blood = GUI:WndFindChildM(_GUIHandle,"Num_blood")
	if _Num_blood ~= 0 then
		GUI:CheckBoxSetAutoChange(_Num_blood,true)
	end
	
	local _ShowTip = GUI:WndFindChildM(_GUIHandle,"ShowTip")
	if _ShowTip ~= 0 then
		GUI:CheckBoxSetAutoChange(_ShowTip,true)
	end
	
	local _ShowCName = GUI:WndFindChildM(_GUIHandle,"ShowCName")
	if _ShowCName ~= 0 then
		GUI:CheckBoxSetAutoChange(_ShowCName,true)
	end
	
	local _DDfengci = GUI:WndFindChildM(_GUIHandle,"DDfengci")
	if _DDfengci ~= 0 then
		GUI:CheckBoxSetAutoChange(_DDfengci,true)
	end
	
	local _DuiDieS = GUI:WndFindChildM(_GUIHandle,"DuiDieS")
	if _DuiDieS ~= 0 then
		GUI:CheckBoxSetAutoChange(_DuiDieS,true)
	end
	
	local _ShowMonster = GUI:WndFindChildM(_GUIHandle,"ShowMonster")
	if _ShowMonster ~= 0 then
		GUI:CheckBoxSetAutoChange(_ShowMonster,true)
	end
	
	local _SlowAction = GUI:WndFindChildM(_GUIHandle,"SlowAction")
	...
	for i = 1 , #SettingBasic.tab do 
		local _Istrue = false
		if CL:GetBasicSetting(SettingBasic.tab[i]) then
			_Istrue = LuaRet
		end
...
function SettingBasic.ShowNameChange(_Handle)
	...
	CL:ChangeBasicSetting(SettingBasic.tab[index],_Istrue)
end
