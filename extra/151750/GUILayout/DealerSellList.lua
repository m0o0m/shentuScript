DealerSellList = {}
function DealerSellList.main()
    local _Parent = LuaGlobal["AttachPartent"]
    local _GUIHandle = 0
    
    _Parent = GUI:WndCreateWnd(_Parent,"DealerSellList",0,0,0)
    if _Parent ~= 0 then
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent,"BTN_DELEGATE_SELL",1832800020,630,455)
    if _GUIHandle ~= 0 then
        GUI:WndSetTextM(_GUIHandle,"我要寄售")
    end
    ...
    DealerSellList.UIInit(_Parent)
end

--UI Logic Code Start
function DealerSellList.UIInit(_Handle)
end
DealerSellList.main()
