DeliveryTaskDlg = {}
function DeliveryTaskDlg.main()
	local _Parent = LuaGlobal["AttachPartent"]
	local _GUIHandle = 0
	_Parent = GUI:WndCreateWnd(_Parent,"DeliveryTaskDlg",1850100032,107,202)
	if _Parent ~= 0 then
	...
	_GUIHandle = GUI:ImageCreate(_Parent,"wndTitle",1850100029,320,13)
	...
	_GUIHandle = GUI:EditCreate(_Parent,"label",150,177,100,32)
	if _GUIHandle ~= 0 then
		GUI:WndSetParam(_GUIHandle, 0)
	...
	if _GUIHandle ~= 0 then
		...
		GUI:WndSetTextColorM(_GUIHandle, 4290950816)
		GUI:ButtonSetDownTextPosChange(_GUIHandle, false)
		GUI:WndRegistScript(_GUIHandle,1002, "DeliveryTaskDlg.CloseWnd")
	end
	...
	DeliveryTaskDlg.UIInit(_Parent)
end

--UI Logic Code Start
function DeliveryTaskDlg.UIInit(_Handle)
	DeliveryTaskDlg.handle = _Handle
	for i = 1, 4 do
		RDItemCtrlSetGUIDataPropByKeyName(_Handle,"item"..i,"霸业神剑",1,false)
	end
end

function DeliveryTaskDlg.CloseWnd()
	GUI:WndClose(DeliveryTaskDlg.handle)
end

DeliveryTaskDlg.main()
