CrystalSelWnd = {}
function CrystalSelWnd:main()
    local _Parent = LuaGlobal["AttachPartent"]
    local _GUIHandle = 0

    _Parent = GUI:WndCreateWnd(_Parent,"CrystalSelWnd",0,270,10)
    if _Parent ~= 0 then
    ...
    if _GUIHandle ~= 0 then
    	GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "CrystalSelWnd.CloseWnd")
    end
    ...
    CrystalSelWnd:UIInit(_Parent)
end

--UI Logic Code Start
CrystalSelWnd.WndHandle = 0
CrystalSelWnd.itemlist = {}
function CrystalSelWnd:UIInit(_Handle)
...
function CrystalSelWnd:item_filter()
...
--蕴魂石列表
function CrystalSelWnd:CreateItemList()
...
function CrystalSelWnd:CreateUI(i)	
	dbg("i"..i)
	...
	if _Parent ~= 0 then
		...
		GUI:WndRegistScript(_Parent, RDWndBaseCL_mouse_lbUp, "CrystalSelWnd._ClickItem")
	end
...
--物品框点击回调事件
function CrystalSelWnd._ClickItem(_Handle)	
    ...
    if _handle ~= 0 then 
		...
		if item_guid ~= 0 and item_guid ~= nil then
			CrystalWnd:FillItemCtr(item_guid);
			GUI:WndClose(CrystalSelWnd.WndHandle)
		end
    end
end	


function CrystalSelWnd.CloseWnd(_Handle)
    if _Handle ~= 0 then 
