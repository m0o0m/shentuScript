LuaUI["装备附魔"].OnInit = function(handle, param, x, y)
	...
	local description = "1.放入装备和10个洗炼石\n2.装备会被随机洗炼出1-3条属性"
	...
	EditSetProp(handle, "GUI_10", nil, nil, false, true, MakeARGB(255, 215, 170, 117), false, "洗炼石")
	...
	if WndPtInWindow(handle, nil, x, y) then
		...
		if guid2 == "0" then
			if not LuaUI["装备附魔"].IsHaveFumoshi() then
				UI:Lua_GenForm("快捷购买#洗炼石")
			return end
			MessageBox(MESSAGE_STYLE_OK, "请放入洗炼石！")
		return end
		local site1, site2
