...
function GetEquipTips(ihandle, ehandle)
	...
	if item == nil then return end; ret = ret..item
	--宝石属性--
	if UI:Lua_GetItemTemplatePropByHandle(ihandle, ITEM_PROP_DRILLABLE) then
		item = tipItemsLayoutSetGem(ihandle, ehandle, true)
		if item == nil then return end; ret = ret..item
	end
...
function tipItemsLayoutBind(ihandle, ehandle, equip)
	...
	local state,color
	if bound then state = "已绑定"; color = 4278276095
	else state = "未绑定"; color = 4281645312 end
...
function tipItemsLayoutRequire(ihandle, ehandle, equip)
	...
	if LuaRet ~= 0 then
		local r_sex = LuaRet
		if not UI:Lua_GetPlayerSelfPropBase(ROLE_PROP_SEX) then return end
		local sex = LuaRet
	...
	if LuaRet ~= 0 then
		local r_atk = LuaRet
		if not UI:Lua_GetPlayerSelfProperty32(ROLE_PROP32_MAX_PHY_ATK) then return end
		local atk = LuaRet
	...
	if LuaRet ~= 0 then
		local r_atk = LuaRet
		if not UI:Lua_GetPlayerSelfProperty32(ROLE_PROP32_MAX_MAG_ATK) then return end
		local atk = LuaRet
	...
	if LuaRet ~= 0 then
		local r_atk = LuaRet
		if not UI:Lua_GetPlayerSelfProperty32(ROLE_PROP32_MAX_TAO_ATK) then return end
		local atk = LuaRet
...
function tipItemsLayoutAttackDefense(ihandle, ehandle, equip)
	...
	if vmin ~= 0 or vmax ~= 0 then
		ret = ret..tipText("物理攻击："..vmin.."-"..vmax.."\n", __color[thigh])
	end
	...
	vmin = vmin + tmin; vmax = vmax + tmax
	if vmin ~= 0 or vmax ~= 0 then
		ret = ret..tipText("魔法攻击："..vmin.."-"..vmax.."\n", __color[thigh])
	end
	...
	vmin = vmin + tmin; vmax = vmax + tmax
	if vmin ~= 0 or vmax ~= 0 then
		ret = ret..tipText("道术攻击："..vmin.."-"..vmax.."\n", __color[thigh])
	end
	...
	vmin = vmin + tmin; vmax = vmax + tmax
	if vmin ~= 0 or vmax ~= 0 then
		ret = ret..tipText("物理防御："..vmin.."-"..vmax.."\n", __color[thigh])
	end
	...
	vmin = vmin + tmin; vmax = vmax + tmax
	if vmin ~= 0 or vmax ~= 0 then
		ret = ret..tipText("法术防御："..vmin.."-"..vmax.."\n", __color[thigh])
	end
...
function tipItemsLayoutFumo(ihandle, ehandle, equip)
	...
	if ehandle == 0 or not equip or sub_type == 5 or sub_type == 14 then return ret end
...
--宝石属性-----------------------------------------------------------------------------------------------
function tipItemsLayoutSetGem(ihandle, ehandle, equip)
	local ret = ""
	local image_t = {1908400010, 1908400010, 1908400010, 1908400010, 1908400010}
	local text_t = {"已打孔\n","已打孔\n","已打孔\n","已打孔\n","已打孔\n",}
	local __bscolor = {
		MakeARGB(255, 255, 0, 255),
		MakeARGB(255, 255, 255, 0),
		MakeARGB(255, 0, 255, 255),
		MakeARGB(255, 0, 255, 0),
		MakeARGB(255, 255, 0, 0)
	}
	local setgem_count=0
	gem = {}
	local func
	if UI:Lua_GetItemEntityCustomVarInt(ehandle, "setgem_count") then
		setgem_count = LuaRet
	end
	if UI:Lua_GetItemEntityCustomVarStr(ehandle, "gem") then
		func = loadstring("gem="..LuaRet)
	end
	if func ~=nil then
		func()
	end

	--if setgem_count~=0 then
		for i = 1, setgem_count do
			if gem[i]~= nil and gem[i][1] ~= "" then
				UI:Lua_GetItemTemplatePropByID(gem[i][1], ITEM_PROP_TIPSICON)
				local itemid=LuaRet
				ret = ret..tipImage(itemid, 20, 20)..tipText(gem[i][2], __bscolor[i])
			else
				ret = ret..tipImage(image_t[i], 20, 20)..tipText(text_t[i], __bscolor[i])
			end
		end
	--end
	
	if ret ~= "" then ret = tipSet(nil, -8)..tipLine()..ret..tipSet(nil, 8) end
	return ret
end
