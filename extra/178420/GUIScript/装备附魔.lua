LuaUI["装备附魔"].OnInit = function(handle, param, x, y)
	...
	local description = "1.请放入10个神化石或10个超级神化石\n2.装备会被随机强化1-3条属性"
	...
	EditSetProp(handle, "GUI_10", nil, nil, false, true, MakeARGB(255, 215, 170, 117), false, "神化石")
	...
	RegisterUIEvent(LUA_EVENT_ONITEMUPDATE, "装备附魔表单反馈", LuaUI["装备附魔"].Update)
	...
	if WndPtInWindow(handle, nil, x, y) then
		...
		if guid2 == "0" then
			if not LuaUI["装备附魔"].IsHaveFumoshi() then
				UI:Lua_GenForm("快捷购买#神化石")
			return end
			MessageBox(MESSAGE_STYLE_OK, "请放入神化石！")
		return end
		local site1, site2
		...
		else msg("神化石或超级神化石不足") end
	end
end

LuaUI["装备附魔"].Update = function()
	local _GUIData = RDItemCtrlGetGUIData(nil, "装备附魔,Item1")
	if UI:Lua_GetItemGUIDataPropByType(_GUIData, ITEMGUIDATA_ITEMGUID) then
		local GUID = LuaRet
		if not RDItemCtrlSetGUIDataPropByGUID(nil, "装备附魔,Item1", GUID) then
...
LuaUI["装备附魔"]["Item1"].OnLButtonDown = function(handle, param, x, y)
	if UI:Lua_GetMouseGUIItemData() then
		...
		if _GUIDataHandle ~= 0 and UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ITEMID) then
			...
			if not UI:Lua_GetItemTemplatePropByID(itemid, ITEM_PROP_SUBTYPE) then return end
			local item_subtype = LuaRet
			if item_type ~= ITEM_TYPE_MATERIAL or item_subtype ~= 3 then
				MessageBox(MESSAGE_STYLE_OK, "请放入神化石！")
			else
				RDItemCtrlFillGUIData(handle, nil, _GUIDataHandle)
			end
...
LuaUI["装备附魔"].ItemMoveIn = function()
	...
	if handle ~= 0 and UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ITEMID) then
		...
		elseif item_type == ITEM_TYPE_MATERIAL and item_subtype == 3 then
