...
function tipItemsLayoutProperty(ihandle, ehandle, equip, notnull)
	...
	--存储状态--
	local jl_types = { {5, 1} }
	local jl_names = {  "聚灵珠","聚灵珠(小)", "聚灵珠(中)", "聚灵珠(大)", "超级聚灵珠", "神级聚灵珠", "坐骑聚灵珠(小)", "坐骑聚灵珠(中)", "坐骑聚灵珠(大)","英雄聚灵珠(小)", "英雄聚灵珠(中)", "英雄聚灵珠(大)" }
	local is_jlz = false
...
function tipItemsLayoutOtherProperty(ihandle, ehandle, equip, notnull)
	...
	if val ~= 0 then ret = ret..tipText("准确："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if val ~= 0 then ret = ret..tipText("魔法躲避："..__pading[val > 0]..val.."\n", __color[thigh]) end
	...
	if UI:Lua_GetItemTemplatePropByHandle(ihandle, ITEM_PROP_SUIT) and LuaRet ~= 0 then
		...
		for site,guid in pairs(data) do
			if UI:Lua_GetItemTemplateHandleByGUID(guid) then
				...
				if tsuitid ~= nil and titemid ~= nil and tsuitid == suitid  then
					--and tsuit[titemid] == nil
					tcount = tcount + 1
		...
		local strs = tipText(name.."("..tcount.."/"..#suitlist..")\n", __color[true])
		
		if name == "沃犸套（战）" or name == "沃犸套（道）" or name == "沃犸套（法）" or name == "祖犸套（法）" or name == "祖犸套（道）" or name == "祖犸套（战）" then
		strs = tipText(name.."("..tcount.."/5)\n", __color[true])
		end
		if name == "赤月套（战）" or name == "赤月套（道）" or name == "赤月套（法）" then
		strs = tipText(name.."("..tcount.."/6)\n", __color[true])		
		end
		ret = ret..strs
		...
		for i = 1,#suit_prop_size do
			if suit_prop_size[i][2] > 0 and suit_prop_value[i][2] > 0 and UI:Lua_GetAttrProp(suit_prop_attr[i][2],ATT_PROP_CHINANAME) then
				...
				str = tipSet(8)..tipText(suit_prop_size[i][2].."/"..#suitlist.." "..GetAttNameValueStr(LuaRet, suit_prop_value[i][2], "提升").."\n", __color[has_attr])
				if name == "沃犸套（战）" or name == "沃犸套（道）" or name == "沃犸套（法）" or name == "祖犸套（法）" or name == "祖犸套（道）" or name == "祖犸套（战）" then
									str = tipSet(8)..tipText(suit_prop_size[i][2].."/5 "..GetAttNameValueStr(LuaRet, suit_prop_value[i][2], "提升").."\n", __color[has_attr])
				end
				if name == "赤月套（战）" or name == "赤月套（道）" or name == "赤月套（法）" then
									str = tipSet(8)..tipText(suit_prop_size[i][2].."/6 "..GetAttNameValueStr(LuaRet, suit_prop_value[i][2], "提升").."\n", __color[has_attr])
				end
									ret = ret..str
			end
		end
...
function tipLayoutFunctionMedal()
	...
	if GetTipParam("ITEMCOUNT") ~= nil then
		...
		if ItemCount > 9 then
			MedalLevel = math.floor(MedalLevel)
		end
	end
