...
LuaUI["装备合成"].OnInit = function(this)
	zbhc.data = {}
	zbhc.type = LuaParam[3]
	local func = loadstring("zbhc.data[1]="..LuaParam[2])
	func()
...
LuaUI["装备合成"]["Ok"].OnLButtonUp = function(this, param, x, y)
	...
	local index = 1
	for i = 1, #t do
		if UI:Lua_GetItemEntityPropByGUID(t[i],ITEM_PROP_EQUIP_REFINELEVEL) then
			local level = LuaRet
	...
	if refine > 0 then
		str = " +"..refine
		if UI:Lua_GetItemEntityPropByGUID(t[index], ITEM_PROP_EQUIP_REFINEDATA) then
			...
			if UI:Lua_GetMouseGUIItemData() then
				...
				if UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ITEMID) then
					...
					if need_id == item_id then
						...
						MessageBox(MESSAGE_STYLE_OK, "#COLORCOLOR_BROWN#放入的装备不合法，请放入#COLORCOLOR_YELLOW#【"..LuaRet.."】")
					end
				end
			else
			...
			if t ~= nil then
				...
				for i = 5, #t do
					...
					if 0 ~= guid and "0" ~= guid then
						if UI:Lua_GetItemEntityPropByGUID(guid, ITEM_PROP_KEYNAME) and LuaRet == t[i] then
							need_clear = false
						end
					end
					if need_clear then
...
LuaUI["装备合成"].OnResult = function()
	if LuaParam[1] ~= "装备合成表单" then return end
	if LuaParam[2] ~= "[装备合成成功！]" then return end
	for i = 1, 10 do
