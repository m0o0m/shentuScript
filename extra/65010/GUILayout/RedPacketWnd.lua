		...
		if nil ~= tInfo[1][1] then
			...
			for i = 1, #tInfo[1] do
				...
				if 0 == math.ceil(i%2) then
					_GUIHandle = GUI:ImageCreate(RedPacketWnd._hLV, "ContentBack"..i, 1808600045, 0, (i - 1)*36)
				else
					_GUIHandle = GUI:ImageCreate(RedPacketWnd._hLV, "ContentBack"..i, 1808600044, 0, (i - 1)*36)
				end
				...
				--local _GUIHandle = GUI:RichEditCreate(RedPacketWnd._hLV,"Content"..i, 2, (i - 1)*34 + 9, 213, 14)
				_GUIHandle = GUI:EditCreate(RedPacketWnd._hLV,"Content"..i, 2, (i - 1)*36 + 9, 213, 14)
				if _GUIHandle ~= 0 then
					GUI:WndSetSizeM(_GUIHandle,85, 14)
					--local str = "#COLORCOLOR_YELLOW#"..tInfo[1][i][12].."元宝#COLORCOLOREND##COLORCOLOR_WHITE#红包#COLORCOLOREND#"
					--GUI:RichEditAppendString(_GUIHandle, tInfo[1][i][1].."送出的"..str)

					GUI:EditSetTextM(_GUIHandle, tInfo[1][i][1])
					GUI:EditSetFontCenter(_GUIHandle)
					GUI:WndSetEnableM(_GUIHandle, false)
				end
				_GUIHandle = GUI:EditCreate(RedPacketWnd._hLV,"Content2"..i, 92, (i - 1)*36 + 9, 213, 14)
				if _GUIHandle ~= 0 then
					GUI:WndSetSizeM(_GUIHandle,150, 14)
					--local str = "#COLORCOLOR_YELLOW#"..tInfo[1][i][12].."元宝#COLORCOLOREND##COLORCOLOR_WHITE#红包#COLORCOLOREND#"
					--GUI:RichEditAppendString(_GUIHandle, tInfo[1][i][1].."送出的"..str)
					GUI:EditSetTextM(_GUIHandle, "送出的"..tInfo[1][i][12].."元宝红包")
					--GUI:EditSetFontCenter(_GUIHandle)
					GUI:WndSetEnableM(_GUIHandle, false)
				end
				_GUIHandle = GUI:ButtonCreate(RedPacketWnd._hLV, "grabBtn"..i, 1808600033, 214, (i - 1)*36 + 3)
				if _GUIHandle ~= 0 then
					GUI:WndSetSizeM(_GUIHandle,30, 30)
					GUI:WndSetParam(_GUIHandle, i)
					GUI:WndRegistScript(_GUIHandle, RDWndBaseCL_mouse_lbClick, "RedPacketWnd.grabBtn_LC")
