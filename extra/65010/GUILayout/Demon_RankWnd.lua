...
local ui = "<form> <dialog OnInit='Demon_RankWnd.UIInit' id='Demon_RankWnd' image='1806200000' x='0' y='25' center='true' w='619' h='460' revmsg='true'>"
	.."	<image id='TestWndChild_1' image='1806200001' x='75' y='0' w='468' h='43' check_point='0' revmsg='true'/>"   --夜袭巫山城排行榜
...
Demon_RankWnd.Wnd = 0
local ranker = deserialize(FormParam[1])
local score = tonumber(FormParam[2])
local ranking = tonumber(FormParam[3])
function Demon_RankWnd.UIInit(_Handle)
	...
	for i=1,#ranker do 
		if i < 11 then 
			...
			if handle ~= 0 then 
				GUI:EditSetTextM(handle,''..ranker[i][2]) 
				GUI:WndSetVisible(handle,true)
			end
			handle = GetWindow(_Handle,"level"..i)
			if handle ~= 0 then 
				GUI:EditSetTextM(handle,''..ranker[i][4]) 
				GUI:WndSetVisible(handle,true)
			end
			handle = GetWindow(_Handle,"job"..i)
			if handle ~= 0 then 
				GUI:EditSetTextM(handle,''..tab_job[tonumber(ranker[i][5])]) 
				GUI:WndSetVisible(handle,true)
			end
			handle = GetWindow(_Handle,"score"..i)
			if handle ~= 0 then 
				GUI:EditSetTextM(handle,''..ranker[i][3]) 
				GUI:WndSetVisible(handle,true)
			end
		end
	end
	...
	if handle ~= 0 then
		if ranking < 6 then
			GUI:EditSetTextM(handle,"恭喜您进入前5名，获得进入皇宫挑战魔王的资格")
		else
			GUI:EditSetTextM(handle,"很遗憾您未进入前5名，无法进入皇宫挑战魔王")
		end
	end
	...
	if handle ~= 0 then
		...
		if ranking < 6 then
			GUI:RichEditAppendString(handle,"#SELECT#进入皇宫#SELECTEND#") 
		else
			GUI:RichEditAppendString(handle,"#SELECT#回到龙城#SELECTEND#") 
		end
	end
	CL:AddDelayTask("Demon_RankWnd.Script()", 1000, 60)
end

function Demon_RankWnd.Script()
	
	Demon_RankWnd.timeimg = Demon_RankWnd.timeimg - 1
	handle = GetWindow(0,"Demon_RankWnd,time")
	if handle ~= 0 then
		GUI:EditSetTextM(handle,Demon_RankWnd.timeimg.."秒")
	end
