...
--UI Logic Code Start
hint = {
	"护符说明：                               1.护符可以极大的增强生命上限             2.护符印记可以通过材料副本、击杀BOSS、回收装备等方式获得                         3.魔符碎片可以通过前往苍月岛击杀BOSS、收集苍月令牌兑换等方式获得                 4.护符不能被强化和注灵",
	"盾牌说明：                               1.盾牌可以极大增加防御能力               2.盾牌残片可以通过材料副本、击杀BOSS、回收装备等方式获得                         3.魔盾碎片可以通过前往苍月岛击杀BOSS、收集苍月令牌兑换等方式获得                 4.升级后盾牌的强化和注灵等级不变",
	"宝珠说明：                               1.宝珠可以极大的增加攻击能力             2.宝珠碎片可以通过材料副本、探宝积分兑换等方式获得                               3.升级后宝珠的强化和注灵等级不变",
	"灵珠说明：                               1.灵珠可以极大的增加暴击率               2.狂暴之灵可以通过材料副本、击杀BOSS、商城购买等方式获得                         3.升级后灵珠的强化和注灵等级不变",
}
str = {
...
function ShenLuWnd.UIInit(_GUIHandle)
	...
	GUI:WndSetSizeM(ShenLuWnd.WndHandle, 434, 503)
	GUI:WndSetMagicUI(ShenLuWnd.WndHandle, 1)
	CenterWnd(ShenLuWnd.WndHandle)
	...
	elseif index == 4 then
		...
		if hunzhu_lv <= 0 then
			...
			if CL:GetItemTemplatePropByHandle(temp1,102) then
				...
				EditSetText(ShenLuWnd.WndHandle,"sz2",""..string.format("%.2f",baojilu_now).."%")
			end
		end
		...
		if CL:GetItemTemplatePropByHandle(temp2,102) then
			...
			EditSetText(ShenLuWnd.WndHandle,"hsz2",""..string.format("%.2f",baojilu_next).."%")
		end
	end
