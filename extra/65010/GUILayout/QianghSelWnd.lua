...
function QianghSelWnd.main()
    ...
    _Parent = GUI:WndCreateWnd(_Parent,"QianghSelWnd",0,380,60)
    if _Parent ~= 0 then
    ...
    if _GUIHandle ~= 0 then
         GUI:WndSetSizeM(_GUIHandle,324, 396)
    end

    _GUIHandle = GUI:EditCreate(_Parent,"Edit0",154,16,100,20)
    if _GUIHandle ~= 0 then
...
function QianghSelWnd.UIInit(_Handle)
  ...
  --[[  for i = 1, 28 do
	...
	if _ClipWndHandle then
		...
		GUI:WndSetSizeM(_ClipWndHandle,340 ,380)
...
function QianghSelWnd.SelItemCtrl(_Handle)
    ...
    local _ParentHandle = GUI:WndGetParentM(QianghSelWnd.WndHandle)
    if _ParentHandle ~= 0 then
        local _IDStr = GUI:WndGetIDM(_ParentHandle)
        if _IDStr == "QianghWnd" then
            QianghWnd.SelItemCtrl(_ItemDataHandle)
        end

         if _IDStr == "ZhuanyWnd" then
