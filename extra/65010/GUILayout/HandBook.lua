...
function HandBook.main()
    ...
    _GUIHandle = GUI:CheckBoxCreate(_Parent,"check",1804000010,"显示全部",593,471)
    if _GUIHandle ~= 0 then
    ...
    if _GUIHandle ~= 0 then
        ...
        GUI:WndSetTextM(_GUIHandle,"万灵志")
    ...
    if _GUIHandle ~= 0 then
        GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "HandBook.KindClick")
        GUI:WndSetTextM(_GUIHandle,"神兵志")
...
function HandBook.SortTb(param, isNormal, showTb)
	local tb = {}	--1名字，2图片，3等级, 4描述，5拥有, 6类型（怪物才有）,7类型名称（怪物才有）
	if param == 1 then
		local temp = {}
...
--一个图片控件设定为DrawCenter时，再更换图片会控件大小会改变，进而使setcenter失效，需要手动调整
