...
function RoleGUINew.main()
    ...
    _GUIHandle = GUI:ImageCreate(_Parent,"RoleGUIBkImg",1805900168,0,0)
    if _GUIHandle ~= 0 then
        GUI:WndSetCanRevMsg(_GUIHandle, false)
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetSizeM(_GUIHandle,662, 550)
    end
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent,"RoleEquimentPage",1805900076,-17,66)
    if _GUIHandle ~= 0 then
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent,"RoleSkillPage",1805900076,-17,121)
    if _GUIHandle ~= 0 then
    ...
    _GUIHandle = GUI:ButtonCreate(_Parent,"RoleFeatsTitlePage",1805900076,-17,176)
    if _GUIHandle ~= 0 then
        GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbClick, "RoleGUINew.RoleFeatsTitleWnd")
        GUI:WndSetTextM(_GUIHandle,"功勋")
        GUI:WndSetParam(_GUIHandle, 0)
        GUI:WndSetTextColorM(_GUIHandle, 4291736418)
        GUI:ButtonSetDownTextPosChange(_GUIHandle, false)
    end

    _GUIHandle = GUI:ButtonCreate(_Parent,"RoleReinPage",1805900076,-17,231)
    if _GUIHandle ~= 0 then
...
function RoleGUINew.ShowChildWnd(index)    --show index-page only
	local childWnd = {"RoleEquiment", "RoleSkill", "RoleFeatsTitle", "RoleRein"}
	for i = 1, #childWnd do
...
function RoleGUINew.RoleFeatsTitleWnd( ... )
    if GetWindow(RoleGUINew.WndHandle,"RoleFeatsTitle")  == 0 then
        UI:Lua_OpenWindow(RoleGUINew.WndHandle, "RoleFeatsTitle.lua")
    end
...
function RoleGUINew.RoleReinWnd()
    ...
    RoleGUINew.ShowChildWnd(4)
    RoleGUINew.ActivePageBtn(4)
end
