...
local ui = "<form> <dialog OnInit='Night_RankWnd.UIInit' id='Night_RankWnd' image='1806200000' x='0' y='25' center='true' w='619' h='460' revmsg='true'>"
	.."	<image id='TestWndChild_1' image='1806200001' x='75' y='0' w='468' h='43' check_point='0' revmsg='true'/>"
...
function Night_RankWnd.main()
	local handle = GetWindow(0,"Night_RankWnd")
	if handle == 0 then 
		GenFormByString(ui)
	end
end
...
function Night_RankWnd.UIInit(_Handle)
	...
	for i=1,#ranker do 
		if i < 11 then 
			...
			if handle ~= 0 then 
				GUI:EditSetTextM(handle,''..ranker[i][1]) 
				GUI:WndSetVisible(handle,true)
			end
	...
	if handle ~= 0 then 
		GUI:EditSetTextM(handle,""..ranking)
	end
	handle = GetWindow(_Handle,"score")
	if handle ~= 0 then 
		GUI:EditSetTextM(handle,""..score)
	end
	
	CL:AddDelayTask("Night_RankWnd.Script()", 1000, 60)
end

function Night_RankWnd.Script()
	
	Night_RankWnd.timeimg = Night_RankWnd.timeimg - 1
	handle = GetWindow(0,"Night_RankWnd,time")
	if handle ~= 0 then
		GUI:EditSetTextM(handle,''..Night_RankWnd.timeimg.."秒")
	end
