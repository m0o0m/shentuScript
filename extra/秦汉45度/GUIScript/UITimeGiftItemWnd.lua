...
function ShowTimeGiftItemWnd()
	if UI:Lua_GetWindow("TimeGiftItemWnd") then
		UI:Lua_WndCloseM()
	end
	TimeGiftItemWnd.Notify_Submit = TimeGiftItemWnd.Notify1
	CreateWindow("TimeGiftItemWnd",true,true)
	SaveWnd()
	if UI:Lua_FindChildM("Close") then
		UI:Lua_WndRegistScript(RDWndBaseCL_mouse_lbUp,"GeneralCloseBtnClick")
		RestoreWnd()
	end
	if UI:Lua_FindChildM("GetGiftBtn") then
		UI:Lua_WndRegistScript(RDWndBaseCL_mouse_lbUp,"GeneralCloseBtnClick")
		RestoreWnd()
	end
	local _ItemCount = #TimeGiftItemWnd.ItemName
	for i=0,8 do
		if UI:Lua_FindChildM("Item"..i) then
			UI:Lua_RDItemCtrlSetItemDataAutoManager(true)
			if i < _ItemCount then
				RDItemCtrlSetGUIDataPropByKeyName(GetCurWindow(), nil, TimeGiftItemWnd.ItemName[i+1],TimeGiftItemWnd.Num[i+1],TimeGiftItemWnd.BindType[i+1]~=0)
			else
	...
	if UI:Lua_FindChildM("InfoRichEdit") then
		...
		elseif TimeGiftItemWnd.FormName == "在线礼包表单" then
			UI:Lua_RichEditAppendString("只要保持在线，就能领取下面的礼物哟~")
		else
			UI:Lua_RichEditAppendString("只要充值  #COLORCOLOR_YELLOW#1#COLOREND#  元，就能领取下面的丰厚礼物哟~")
		end
...
function TimeGiftItemWnd_LbuttonUp(_Handle,Param,X,Y)
	...
	if not UI:Lua_RichEditGetSelectClickString() then return end
	if LuaRet == "我要充值" then
...
function TimeGiftItemWnd_OnShowForm()
	if LuaParam[1] == "在线礼包表单" or LuaParam[1] == "充值礼包表单" or LuaParam[1] == "首充礼包表单" then
		TimeGiftItemWnd.Clear()
		TimeGiftItemWnd.FormName = LuaParam[1]
		local _FormData = strsplit(LuaParam[2],"#")
