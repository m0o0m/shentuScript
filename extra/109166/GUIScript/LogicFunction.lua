--
-- Author: miyezng
-- Date:   2016-09-23 13:27:57
-- Last Modified by:   miyezng
-- Last Modified time: 2016-09-23 13:29:22
--
--Lua相关设置
function LoadLuaClientCfg()
    ...
    LuaGlobal["UseNewWorldMap"]         = true  --是否用新版的世界地图
    LuaGlobal["NpcFont"]           = "system"    --NPC头顶字体
    LuaGlobal["NpcFontName"] = "system" --npc名字的字体
    LuaGlobal["NpcNameColor"] = 4286185269--npc名字的颜色
    LuaGlobal["NpcNameVOffset"] = 0   --npc名字的垂直方向偏移
    LuaGlobal["TipsBackImgID_Normal"] = "1913780004,1913780006,1913780005,1913780007,1913780008,1913780000,1913780001,1913780002,1913780003" --物品装备Tips的背景图片,空为默认
    LuaGlobal["TipsBackImgID_Focus"] = "1913790004,1913790006,1913790005,1913790007,1913790008,1913790000,1913790001,1913790002,1913790003" --当前物品Tips背景图片
    LuaGlobal["TipsDrawGrow"] = 0   --物品tips文本是否描边
    LuaGlobal["ManShadowID"]        = 0   --是否配置男影子 ，0不配置
    LuaGlobal["WomenShadowID"]        = 0   --是否配置女影子 ，0不配置
    LuaGlobal["PlayerDigImage"]        = 1  --是否配置头顶有挖肉图标
    ...
    LuaGlobal["DrawHairBehindWall"] = true  --墙后面是否绘制头发
    LuaGlobal["UseItemNoDelay"] = false
    LuaGlobal["BigMapWindow_JumpNameTxtColor"] = MakeARGB(255,234,209,177)	--大地图跳转点文本的颜色
    LuaGlobal["RoleNameX"] = 220 --
    LuaGlobal["RoleNameY"] = 27--
    LuaGlobal["DrawSelectHalo"] = true --是否显示怪物选中时候的脚底光环
    LuaGlobal["StallImage"] = 1902000060 --摆摊显示的图片
    LuaGlobal["UseCampColor"] = false --是否启用阵营颜色
    LuaGlobal["GameMallVIPTip"] = "在商城中使用元宝购买商城物品可以获得积分（每花费1元宝，获得1积分）"
    LuaGlobal["DrawWingOnHorse"] = true --骑坐骑时是否描画翅膀
    LuaGlobal["PlayerTextPosOffset"] = -8    --人物血条位置
    LuaGlobal["ShowDigWithStand"] = 0	--是否使用挖肉动作
    LuaGlobal["ReconnetType"] = 0 --断线时是否重连，默认0 重连，1 则退出
    LuaGlobal["BigMapWindow_JumpNameFont"] = "system"	--大地图跳转点文本的字体
    LuaGlobal["NpcTalkBkAroundImg"] = "1913100001, 1913100003, 1913100005, 1913100007, 1913100009, 1913100002, 1913100006, 1913100004, 1913100008,1903700043"   --npctalk面板上,自定义面板大小时九宫格背景.前九张为九宫格,第十张为分割线
	LuaGlobal["NpcTalkBkNameBottomImg"] = "1913800004"	--npctalk面板,npc名字底部背景图
	LuaGlobal["NpcTalkBKNoTaskImg"] = "1913800003"	--npctalk面板,没有任务时所用的单张背景图
	LuaGlobal["NpcQuestHeadImage"] = "1044800000, 1044900001, 1045000000"	--npc头顶任务图标,1：任务可交，2：任务可接，3：任务已接不可交
	LuaGlobal["NpcTalkBkTaskImg"] = "1913800004，1913800009"	--npctalk面板,有任务时背景图和选中时高亮图片
        LuaGlobal["DLImageType"] = 1	--下载框
        LuaGlobal["StallImage"] = 1914100001 --摆摊显示的图片
...
function GetEquipmentEnhanceDesc()
    local _Desc = ""
    _Desc = _Desc.."#OFFSET<X:-10,Y:0>#    注意：装备精炼失败时会掉到0级\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0>#          辅助道具介绍\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0>#  幸运星：精炼时放入，可以提升成功率\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0>#  护炼符：精炼时放入，可保证精炼失败\n后，装备精炼等级最多下降1级\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0>#         #COLORCOLOR_ORANGE#┌──────┐\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0>#         │精炼属性介绍│\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0># ┌───┴──────┴────┐\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0># │#COLOREND##COLORCOLOR_YELLOW#精炼1~8级：#OFFSET<X:1,Y:0>#每级属性随机+1%~2%#OFFSET<X:3,Y:0>##COLOREND##COLORCOLOR_ORANGE##OFFSET<X:2,Y:0>#│\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0># ├───────────────┤\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0># │#COLOREND##COLORCOLOR_YELLOW#精炼9~11级：每级属性随机+1%~3%#COLOREND##COLORCOLOR_ORANGE#│\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0># ├───────────────┤\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0># │#COLOREND##COLORCOLOR_YELLOW#精炼12以上：每级属性随机+2%~4%#COLOREND##COLORCOLOR_ORANGE#│\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:0># └───────────────┘\n"
    _Desc = _Desc.."#OFFSET<X:50,Y:0>##COLORCOLOR_ORANGE#┌────────┐\n"
    _Desc = _Desc.."#OFFSET<X:51,Y:-5>#│全套精炼属性介绍│\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># ┌───┴──┬─────┴───┐\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># │全套精炼+6#OFFSET<X:12,Y:0>#│额外附加：属性+1%#OFFSET<X:2,Y:0>##OFFSET<X:4,Y:0>#│\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># ├──────┼─────────┤\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># │全套精炼+9#OFFSET<X:12,Y:0>#│额外附加：属性+2%#OFFSET<X:1,Y:0># #OFFSET<X:-1,Y:0>#│\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># ├──────┼─────────┤\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># │全套精炼+12#OFFSET<X:6,Y:0>#│额外附加：属性+4%#OFFSET<X:0,Y:0># │\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># ├──────┼─────────┤\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># │全套精炼+15#OFFSET<X:6,Y:0>#│额外附加：属性+7%#OFFSET<X:1,Y:0># #OFFSET<X:-1,Y:0>#│\n"
    _Desc = _Desc.."#OFFSET<X:-10,Y:-5># └──────┴─────────┘#COLOREND#\n"
    return _Desc
end
...
function UserInit()
	UI:Lua_GetWindow("GameMainInterface,EquimentNoticWindow")
        ...
        UI:Lua_SetDownLoadMessageInfo(50, 67, "GREENG")
end