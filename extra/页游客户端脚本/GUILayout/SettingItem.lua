...
function SettingItem.UIInit(_GUIHandle)
	...
	for i = 1, 6 do
		for j = 1, 3 do
			local posX = (j - 1) * 135
			local posY = (i - 1) * 37
			local _check = GUIWndFindChildM(_GUIHandle,"check"..i..j)
			if _check ~= 0 then
				GUIWndSetParam(_check, j)
				GUIWndAddProperty(_check, "hang", ""..i)
	...
	local _ComboBox = GUIWndFindChildM(_GUIHandle,"ComboBox1")
	if _ComboBox ~= 0 then
		GUIComboBoxClear(_ComboBox)
		GUIComboBoxSetScrollBarImage(_ComboBox, 1800000085, 1800000001, 1800000089, 1800000005)
		GUIComboBoxSetListLength(_ComboBox, 120)
		GUIComboBoxAddString(_ComboBox, "全部", 0, _color[false])
		GUIComboBoxAddString(_ComboBox, "其他", 1, _color[false])
		GUIComboBoxAddString(_ComboBox, "武器", 2, _color[false])
		GUIComboBoxAddString(_ComboBox, "衣服", 3, _color[false])
		GUIComboBoxAddString(_ComboBox, "头盔", 4, _color[false])
        GUIComboBoxAddString(_ComboBox, "项链", 5, _color[false])
        GUIComboBoxAddString(_ComboBox, "护腕", 6, _color[false])
		GUIComboBoxAddString(_ComboBox, "戒指", 7, _color[false])
		GUIComboBoxAddString(_ComboBox, "靴子", 8, _color[false])
		GUIComboBoxAddString(_ComboBox, "勋章", 9, _color[false])		  
				
	end

	
	local _Istrue = false
	if CLGetBasicSetting(4004) then
		_Istrue = as3.tolua(LuaRet)
	end
	local _handle = GetWindow(nil,"GameSettingDlg,SettingItem,AllGet")
	GUICheckBoxSetCheck(_handle,_Istrue)
	_Istrue = false
	if CLGetBasicSetting(4005) then
		_Istrue = as3.tolua(LuaRet)
	end
...
function item_update(index,str)
	if str ~= "" and str ~= nil then
		if CLGetItemListByString(str) then
			SettingItem.item = getTable(as3.tolua(LuaRet))	
		end
		...
		for i = x , y do 	
			...
			for j = 1 , 3 do 
				...
				GUICheckBoxSetCheck(_handle,SettingItem.item[i][j])			
			end	
			local _handle = GetWindow(nil,"GameSettingDlg,SettingItem,Edit"..a)
			local str = as3.tolua(SettingItem.item[i][0])
			EditSetText(_handle,nil,str)		
			a = a + 1		
		end
	else
	
		if CLGetItemListByType(index,SettingItem.page) then
			SettingItem.item = getTable(as3.tolua(LuaRet))	
		end
...
function SettingItem.add_time1(_handle)
	...
	GUIWndAddTimerBrief(_handle, 1000, "SettingItem.down_page")	
end	

function SettingItem.add_time(_handle)
	SettingItem.up_page(_handle)
	GUIWndAddTimerBrief(_handle, 1000, "SettingItem.up_page")	
end	
...
function SettingItem.deltime(_handle)
	GUIWndDelAllTimer(_handle)
end	

function SettingItem.down_page(_handle)
	SettingItem.page = SettingItem.page + 1
	if SettingItem.str ~= "" then
		if CLGetItemListByString(SettingItem.str) then
			SettingItem.item = getTable(as3.tolua(LuaRet))
		...
		if not CLGetItemListByType(SettingItem.index,SettingItem.page) then
			SettingItem.page = SettingItem.page - 1
			return
		end
	end	
...
function SettingItem.EditChange(_handle)
	SettingItem.str = GUIEditGetTextM(_handle)
	SettingItem.page = 1
	item_update(SettingItem.index,SettingItem.str)
end
...
function SettingItem.All(_Handle)
	...
	CLChangeBasicSetting(tab[index],_Istrue)
	item_update(SettingItem.index)		
end
function SettingItem.check(_Handle)
	...
	local str = GUIEditGetTextM(_handle)
	if str ~= "" then 
		CLSetItemPropByName(str,tab[index],_Istrue) 
	end	
end
