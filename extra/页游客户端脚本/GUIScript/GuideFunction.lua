...
f.TaskClick = function()
	local text = as3.tolua(LuaParam[1])
	f.trigger("追踪点击事件", text)
end

f.OpenNpc = function()
	local npc_name = as3.tolua(LuaParam[1])
...
f.OnOpenDlg = function()
	...
	local path = as3.tolua(LuaParam[1])
...
f.ClickNpcText = function()
	local text = as3.tolua(LuaParam[2])
...
f.done = function(id)
	...
	if not as3.tolua(UI:Lua_GetGameEventFlag(flag)) then
		log(this_file.."done函数调用失败，Lua_GetGameEventFlag函数失败")
		return true
	end
	
	return (as3.tolua(LuaRet) == 1)
end
...
f.open1 = function(id)
	...
	if nil ~= tbl[6] and 0 ~= tbl[6] and as3.tolua(UI:Lua_IsTextureExists(tbl[6])) then
	...
	local girl_height = 0
	if tbl[6] ~= 0 then
		if as3.tolua(UI:Lua_GetTextureWidth(tbl[6])) then
			girl_width = as3.tolua(LuaRet)
		end
		if as3.tolua(UI:Lua_GetTextureHeight(tbl[6])) then 
			girl_height = as3.tolua(LuaRet)
		end
...
f.open2 = function(id)
	...
	if not as3.tolua(UI:Lua_AddMagicToPoint(magic, x, y, speed, time)) then
		return false
	end
...
f.PkgRectifyPos = function(this, item)
	...
	while true do
		...
		if not as3.tolua(UI:Lua_GetItemTemplateHandleByKeyName(item)) then
			break
		end
		
		local handle = as3.tolua(LuaRet)
		if not as3.tolua(UI:Lua_GetItemTemplatePropByHandle(handle, ITEM_PROP_ID)) then
			break
		end
		local itemid = as3.tolua(LuaRet)
		
		if not as3.tolua(UI:Lua_PackageFindItemPosById(itemid)) then
			break
		end
		local page = as3.tolua(LuaRet[1])
		local pos = as3.tolua(LuaRet[2])
		
		if not as3.tolua(UI:Lua_PackageActivePage(page)) then
			break
		end
...
f.PkgRectifyPosForSkill = function(this)
	...
	elseif as3.tolua(LuaRet) == 2 then
		keyname = "炎球术"
	elseif as3.tolua(LuaRet) == 3 then
		keyname = "治疗咒"
	end
