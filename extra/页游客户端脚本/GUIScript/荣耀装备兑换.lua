...
LuaUI["荣耀装备兑换"].OnInit = function(this)
	rongyao.data = {}
	local func = loadstring("rongyao.data[1]="..as3.tolua(LuaParam[2]))
	func()
...
LuaUI["荣耀装备兑换"]["Ok"].OnLButtonUp = function(this, param, x, y)
	...
	for k, v in pairs(t) do
		...
		return end
		if not bind then
			if as3.tolua(UI:Lua_GetItemEntityPropByGUID(v, ITEM_PROP_ENTITY_IS_BOUND)) and true == as3.tolua(LuaRet) then
				bind = true
			end
		end
	end
			...
			if as3.tolua(UI:Lua_GetMouseGUIItemData()) then
				...
				if as3.tolua(UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ITEMID)) then
					...
					if need_id == item_id then
						...
						MessageBox(MESSAGE_STYLE_OK, "#COLORCOLOR_BROWN#放入的道具不合法，请放入#COLORCOLOR_YELLOW#【"..as3.tolua(LuaRet).."】")
					end
				end
			else
			...
			if t ~= nil then
				...
				for i = 5, #t do
					...
					if 0 ~= guid and "0" ~= guid then
						if as3.tolua(UI:Lua_GetItemEntityPropByGUID(guid, ITEM_PROP_KEYNAME)) and as3.tolua(LuaRet) == t[i][1] then
							need_clear = false
						end
					end
					if need_clear then
...
LuaUI["荣耀装备兑换"].OnResult = function()
	...
	if as3.tolua(LuaParam[2]) ~= "[兑换荣耀装备成功！]" then return end
	for i = 1, 10 do
