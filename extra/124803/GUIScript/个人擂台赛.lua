...
LuaUI["个人擂台赛"].OnInit = function(handle)
	Lts_swnd_data_init()
	WndCalSize(handle)
	CenterWnd(handle)
	...
	RegisterUIEvent(LUA_EVENT_SUBMITFORMACK, "个人擂台赛表单观战反馈", LuaUI["个人擂台赛"].OnResult)
...
LuaUI["个人擂台赛"].OnClose = function(handle)
	UnRegisterUIEvent(LUA_EVENT_SUBMITFORMACK, "个人擂台赛表单观战反馈")
	MenuDeleteMenu(handle, nil)
end

LuaUI["个人擂台赛"]["Close"].OnLButtonUp = function(handle, param, x, y)
	if WndPtInWindow(handle, nil, x, y) then WndClose(handle, "parent") end
end

LuaUI["个人擂台赛"]["PrevPage"].OnLButtonUp = function(handle, param, x, y)
	...
	cur_page = cur_page - 1
	--msg("请求页数："..cur_page)
	UI:Lua_SubmitForm("个人擂台赛表单", "fanye", tostring(cur_page))
	Lts_swnd.power = false
end

LuaUI["个人擂台赛"]["NextPage"].OnLButtonUp = function(handle, param, x, y)
	if not WndPtInWindow(handle, nil, x, y) then return end
	--回调服务器脚本
...
LuaUI["个人擂台赛"]["Ok"].OnLButtonUp = function(handle, param, x, y)
	if not WndPtInWindow(handle, nil, x, y) then return end
	--回调服务器脚本
	local state = Lts_swnd.state
	if state == nil or true == state then return end
	--msg("个人擂台赛表单.lua gerenbaoming("..guid..")")
	UI:Lua_SubmitForm("个人擂台赛表单", "gerenbaoming", "")
end

LuaUI["个人擂台赛"]["Cancel"].OnLButtonUp = function(handle, param, x, y)
	if not WndPtInWindow(handle, nil, x, y) then return end
	--回调服务器脚本
	local state = Lts_swnd.state
	if state == nil or false == state then return end
	--msg("个人擂台赛表单.lua gerenquxiao("..guid..")")
	UI:Lua_SubmitForm("个人擂台赛表单", "gerenquxiao", "")
end
...
function Lts_swnd_wnd_info_mouse_in(handle)
	WndSetVisible(nil, "个人擂台赛,"..WndGetID(handle).."On", true)
end

function Lts_swnd_wnd_info_mouse_out(handle)
	WndSetVisible(nil, "个人擂台赛,"..WndGetID(handle).."On", false)
end
...
function Lts_swnd_wnd_info_menu_show(handle, param, x, y)
	...
	local hMenu = MenuGetMenu(nil, "个人擂台赛")
...
function Lts_swnd_wnd_look_button_up(handle, param, x, y)
	...
	if name == nil then return end
	--msg("观看玩家："..name)
	UI:Lua_SubmitForm("个人擂台赛表单", "guanzhan", name)
end


-----------------------------------------------------------------------------------------
LuaUI["个人擂台赛"].OnResult = function()
	if LuaParam[1] ~= "个人擂台赛表单" then return end
	if LuaParam[2] == "" then WndClose(nil, "个人擂台赛") end
end

function Lts_swnd_data_init()
	--msg(LuaParam[2])
	--#当前页#总页数#是否报名#昵称，等级，职业，是否战斗中
	local str = string.gsub(LuaParam[2], "^#", "") --去掉第一个#字符
...
function Lts_swnd_wnd_init()
	local handle = GetWindow(nil, "个人擂台赛")
