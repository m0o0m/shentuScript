...
function LoadLuaClientCfg()
	...
	-- LuaGlobal["NpcTalkBkAroundImg"] = "1800000334,1800000335,1800000336,1800000337,1800000342,1800000338,1800000339,1800000340,1800000341,1903700043"
...
function win_msg_up(str)
	...
	local _GUIHandle = GUI:RichEditCreate(0, "prompt_msg", _DeviceSizeX/2 - 180, _DeviceSizeY/2 - 300, 320, 32);
	RichEditSetProp(0, "prompt_msg", true, false, false, "SIMLI", 0xFF00ff00, str);
	prompt_msg_count = 0;
	GUI:WndAddTimerBrief(_GUIHandle, 40, "MovePromptEditWnd");
...
function MovePromptEditWnd()
	...
	if _handle ~= 0 then 
		...
		local posX = LuaRet[1];
		local posY = LuaRet[2];
		GUI:WndSetPosM(_handle, posX, posY - 2);
