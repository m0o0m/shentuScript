...
function GetEquipTips(ihandle, ehandle)
	...
	--原名显示--
	item = tipItemsLayoutSources(ihandle, ehandle, true)
...
function tipItemsLayoutTitle(ihandle, ehandle, equip)
	...
	if BigIcon ~= 0 and BigIcon ~= nil then
		ret = ret..tipSet2(0,30)..tipImage(1803100002,58,58)..tipSet2(4,34)..tipImage(BigIcon, 50, 50)
	end	
...
function tipItemsLayoutBind(ihandle, ehandle, equip)
	...
	if ehandle ~= 0 then
		if GetTipParam("TYPE") == 1 then 
		if not UI:Lua_GetItemEntityPropByHandle(ehandle, ITEM_PROP_ENTITY_SITE) then return "err 436"end
		local site = tonumber(LuaRet)
		
		if equip ~= 0 and ehandle and ower ~= 0 and self == ower  then
			if EQUIMENT_POS == GetItemPosType(site) then
				ret = ret..tipSet2(147,20)..tipImage(1820800001)
			else
				ret = ret..tipSet2(147,20)..tipImage(1820800002)
			end	
		end
	end
end
	...
	if 	GetTipParam("TYPE") == 1 then 
	if bound then
		ret = ret..tipSet2(147,50)..tipImage(1820800004)
			else
		ret = ret..tipSet2(147,50)..tipImage(1820800005)
		end
		else 
			if bound then
		ret = ret..tipSet2(147,20)..tipImage(1820800004)
			else
		ret = ret..tipSet2(147,20)..tipImage(1820800005)
		end
		end
	end
...
function tipItemsLayoutZhuangbeiJieshao(ihandle, ehandle, equip)
	...
	if ret ~= "" then ret = tipLine()..tipSet(nil, -2)..ret..tipSet(nil, 8) end; return ret
end
...
function tipItemsLayoutSources(ihandle, ehandle, equip)
	local ret = ""
	if ehandle == 0 then return ret end
	local str_t = {"","","",""}
	local drop,make = 0,0
	if UI:Lua_GetItemEntityCustomVarInt(ehandle,"drop") then
		drop = LuaRet	
	end
	if UI:Lua_GetItemEntityCustomVarInt(ehandle,"make") then
		make = LuaRet	
	end
	if drop == 1 then
		ret = ret..tipText("[物品来源]\n\n", 4280602623)
		for i = 1,4 do 
			if UI:Lua_GetItemEntityCustomVarStr(ehandle,"drop"..i) then
				str_t[i] = LuaRet
			end
		end
		ret = ret..tipSet(10)..tipText("地图: "..str_t[1].."\n", 4286185269)
		ret = ret..tipSet(10)..tipText("怪物: "..str_t[2].."\n", 4286185269)
		ret = ret..tipSet(10)..tipText("击杀: "..str_t[3].."\n", 4286185269)
		ret = ret..tipSet(10)..tipText("时间: "..str_t[4].."\n", 4286185269)
	elseif make == 1 then
		ret = ret..tipText("[物品来源]\n\n", 4280602623)
		for i = 1,3 do 
			if UI:Lua_GetItemEntityCustomVarStr(ehandle,"make"..i) then
				str_t[i] = LuaRet
			end
		end
		ret = ret..tipSet(10)..tipText("物品源: "..str_t[1].."\n", 4286185269)
		ret = ret..tipSet(10)..tipText("制造者: "..str_t[2].."\n", 4286185269)
		ret = ret..tipSet(10)..tipText("时  间: "..str_t[3].."\n", 4286185269)
	else
		return ret
	end
	if ret ~= "" then ret = tipSet(nil, -8)..tipLine()..ret..tipSet(nil, 8) end; return ret
