...
function ExchangeItem(npc, player)
	--开始开启进度条
	if lualib:ProgressBarStart(player, 5000, "绝路城宝箱", "progress_complete", "progress_abort", npc) == false then
		return "出错了，请联系GM@2！"
	end
...
function progress_complete(player, npc)		--进度条成功
	...
	--宝箱组的概率
	local item_group_probability = {0, 100, 6500, 0, 0, 0}

	--宝箱组：
	--金币：最少数量   最多数量
	--元宝：数量
	--物品：物品keyname	 数量	掉落概率起始点	掉落概率结束点	是否公告
	local item_group = {}
	item_group[1] = {3000, 10000}
	item_group[2] = {100}

	item_group[3] = {}
	item_group[3][1] = {"钱袋(1万)",   1, 1,    3513,  1}
	item_group[3][2] = {"钱袋(2万)",   1, 3514, 5269,  1}
    item_group[3][3] = {"钱袋(5万)",   1, 5270, 5972,  1}
    item_group[3][4] = {"钱袋(10万)",  1, 5973, 6323,  1}
    item_group[3][4] = {"钱袋(20万)",  1, 6324, 6000,  1}

	item_group[4] = {}
	item_group[4][1] = {"召唤石", 4, 1,    2510,  0}
	item_group[4][2] = {"召唤石", 5, 2511, 4518,  0}
	item_group[4][3] = {"召唤石", 6, 4519, 6191,  0}
	item_group[4][4] = {"召唤石", 7, 6192, 7625,  0}
	item_group[4][5] = {"召唤石", 8, 7626, 8880,  0}
	item_group[4][6] = {"召唤石", 9, 8881, 10000, 0}
	...
	if really_group[1] == true then
        ...
        ret = lualib:Player_AddGold(player, gold_value, false, "加金币：绝路城宝箱", player)
		if ret == false then
			lualib:SysMsg_SendWarnMsg(player, "系统错误，请联系GM@1！")
			lualib:Npc_Remove(npc)
			return
		else
			lualib:SysMsg_SendPromptMsg(player, "恭喜你，开启绝路城宝箱获得"..gold_value.."金币！")
		end
	end

	--掉落元宝
	if really_group[2] == true then
        ret = lualib:Player_AddIngot(player, item_group[2][1], true, "加元宝：打开绝路城宝箱", player)
		if ret == false then
			lualib:SysMsg_SendWarnMsg(player, "系统错误，请联系GM@2！")
			lualib:Npc_Remove(npc)
			return
		else
			lualib:SysMsg_SendPromptMsg(player, "恭喜你，开启绝路城宝箱获得"..item_group[2][1].."绑定元宝！")
			lualib:SysMsg_SendCenterMsg(1, "玩家"..lualib:Player_GetStrProp(player,  lua_role_user_name).."开启绝路城宝箱获得"..item_group[2][1].."绑定元宝！", "")
		end
	end
		...
		if really_group[i] == true then
			...
			for j = 1, table.getn(item_group[i]) do
				if probability_value >= item_group[i][j][3] and probability_value <= item_group[i][j][4] then
					...
					ret = lualib:Player_GiveItemUnbind(player, item_group[i][j][1], item_group[i][j][2], "给物品：绝路城宝箱", player)
					if ret == false then
					...
					if item_group[i][j][5] == 1 then
						lualib:SysMsg_SendCenterMsg(1, "玩家"..lualib:Player_GetStrProp(player,  lua_role_user_name).."开启绝路城宝箱获得"..item_group[i][j][1].."*"..item_group[i][j][2], "")
						lualib:SysMsg_SendBroadcastColor("玩家#COLORCOLOR_BROWN#["..lualib:Player_GetStrProp(player,  lua_role_user_name).."] #COLORCOLOR_YELLOW#开启绝路城宝箱获得#COLORCOLOR_BROWN#"..item_group[i][j][1].."*"..item_group[i][j][2], "", 1, 12)
					end
					break
				end
			end
