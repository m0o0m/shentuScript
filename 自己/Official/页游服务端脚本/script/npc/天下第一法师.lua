...
function main(npc, player)
	...
	msg = msg.."#OFFSET<X:0,Y:0>##IMAGE<ID:1902700042>##OFFSET<X:0,Y:0>#<@Nothing *01*拜见天下第一法师>\n"
	return msg
end
...
function npc_update(npc)
	if not (lualib:IO_FileExists(record_file[2])) then
		lualib:Npc_ChangeName(npc, record_name[2])
		return
	end
	
	local texts = lualib:IO_GetFileText(record_file[2])
	if texts == nil then
		lualib:Npc_ChangeName(npc, record_name[2])
	else
