...
function ExchangeItem(npc, player)
	--开始开启进度条
	if lualib:ProgressBarStart(player, 5000, "秘境沉船宝箱", "progress_complete", "progress_abort", npc) == false then
		return "出错了，请联系GM@2！"
	end
...
function progress_complete(player, npc)		--进度条成功
	...
	--宝箱组的概率
	local item_group_probability = {0, 100, 7500, 0, 0, 0}
	...
	item_group[3] = {}
	item_group[3][1] = {"钱袋(1万)",  1, 1,     3989,  1}
	item_group[3][2] = {"钱袋(2万)",  1, 3990,  5983,  1}
    item_group[3][3] = {"钱袋(5万)",  1, 5984,  6780,  1}
	item_group[3][4] = {"钱袋(10万)", 1, 6781,  7179,  1}
    item_group[3][5] = {"钱袋(20万)", 1, 7180,  7377,  1}
	item_group[3][6] = {"钱袋(50万)", 1, 7378,  7457,  1}
    item_group[3][7] = {"金条",       1, 7458,  7500,  1}

	item_group[4] = {}
	item_group[4][1] = {"召唤石", 6,  1,    2365,  0}
	item_group[4][2] = {"召唤石", 7,  2366, 4335,  0}
	item_group[4][3] = {"召唤石", 8,  4336, 6024,  0}
	item_group[4][4] = {"召唤石", 9,  6025, 7502,  0}
	item_group[4][5] = {"召唤石", 10, 7503, 8815,  0}
	item_group[4][6] = {"召唤石", 11, 8816, 10000, 0}
	...
	if really_group[1] == true then
		...
		if ret == false then
			...
			lualib:SysMsg_SendPromptMsg(player, "恭喜你，开启秘境沉船宝箱获得"..gold_value.."金币！")
		end
	end

	--掉落元宝
	if really_group[2] == true then
        ret = lualib:Player_AddIngot(player, item_group[2][1], true, "加元宝：打开秘境沉船宝箱", player)
		if ret == false then
			lualib:SysMsg_SendWarnMsg(player, "系统错误，请联系GM@2！")
			lualib:Npc_Remove(npc)
			return
		else
			lualib:SysMsg_SendPromptMsg(player, "恭喜你，开启秘境沉船宝箱获得"..item_group[2][1].."绑定元宝！")
			lualib:SysMsg_SendCenterMsg(1, "玩家"..lualib:Player_GetStrProp(player,  lua_role_user_name).."开启秘境沉船宝箱获得"..item_group[2][1].."绑定元宝！", "")
		end
	end
		...
		if really_group[i] == true then
			...
			for j = 1, table.getn(item_group[i]) do
				if probability_value >= item_group[i][j][3] and probability_value <= item_group[i][j][4] then
					...
					ret = lualib:Player_GiveItemUnbind(player, item_group[i][j][1], item_group[i][j][2], "给物品：秘境沉船宝箱", player)
					if ret == false then
					...
					if item_group[i][j][5] == 1 then
						lualib:SysMsg_SendCenterMsg(1, "玩家"..lualib:Player_GetStrProp(player,  lua_role_user_name).."开启秘境沉船宝箱获得"..item_group[i][j][1].."*"..item_group[i][j][2], "")
						lualib:SysMsg_SendBroadcastColor("玩家#COLORCOLOR_BROWN#["..lualib:Player_GetStrProp(player,  lua_role_user_name).."] #COLORCOLOR_YELLOW#开启秘境沉船宝箱获得#COLORCOLOR_BROWN#"..item_group[i][j][1].."*"..item_group[i][j][2], "", 1, 12)
					end
					break
				end
			end
