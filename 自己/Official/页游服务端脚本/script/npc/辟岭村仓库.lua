...
gQuestMap =
{
   {1113, "", "QuestAccomplish1113","", "*01*完成任务: 包裹和高级仓库扩充"},
...
local talk_t = {"我是仓库管理员……我是仓库管理员……我是仓库管理员……我这是在记住自己的职责。", "仓库管理员是做什么的？这你也问得出来？你是在侮辱我的智商还是侮辱你的智商？", "巫山城存的东西，能在龙城取出来，真是神奇。"}
...
function main(npc, player)
    local msg = "     #COLORCOLOR_GREENG#我这儿可以储存物品喔……#COLOREND#你的包裹满了吗？赶紧存到我这儿来吧！有我负责仓库的看管，不仅安全，而且免费哦！\n"
	...
	msg = msg.."#OFFSET<X:0,Y:10>##IMAGE1902700037#<@InfoWarehouse *01*[打开仓库]>\n"
	...
	msg = msg.."                                                      #OFFSET<X:0,Y:2>##IMAGE1902700034##OFFSET<X:0,Y:-2>#<@Lever *01*[离开]>"
...
function Jin_DH(npc_guid, player_guid)
	...
	msg = msg.."将金条兑换成金币需要2000金币的手续费\n"
	msg = msg.."#COLORCOLOR_YELLOW#━━━━━━━━━━━━━━━━━━━━━━━━━━━━━#COLOREND#\n \n"
	msg = msg.."#OFFSET<X:0,Y:2>##IMAGE1902700037##OFFSET<X:0,Y:-2>#<@ExchangeGold *01*[将金币「兑换」成金条]>\n \n"
	msg = msg.."#OFFSET<X:0,Y:-5>##IMAGE1902700037##OFFSET<X:0,Y:-2>#<@ExchangeGoldEx *01*[将金条「兑换」成金币]>\n \n \n"
...
function ExchangeGold(npc_guid, player_guid)
    ...
    if not lualib:Player_SubGold(player_guid, gold_value, false, "扣金币：辟岭村仓库将金币「兑换」成金条", "辟岭村仓库") then
        return "扣除金币失败！\n \n \n \n \n \n \n \n \n                                                  #OFFSET<X:0,Y:2>##IMAGE1902700019##OFFSET<X:0,Y:-2>#<@Jin_DH *01*「返回」>"
    end

	local result = lualib:Player_GiveItemUnbind(player_guid, "金条", 1, "给物品：辟岭村仓库", player_guid)
	...
	return "「兑换」金条成功！\n \n \n \n \n#OFFSET<X:0,Y:1>##IMAGE1902700037##OFFSET<X:0,Y:-1>#<@ExchangeGold *01*[继续兑换]>\n \n \n                                                  #OFFSET<X:0,Y:2>##IMAGE1902700019##OFFSET<X:0,Y:-2>#<@Jin_DH *01*「返回」>"
end

function ExchangeGoldEx(npc_guid, player_guid)
	local count = lualib:Player_GetItemCount(player_guid, gold_)
	if count < 1 then
		return "没有找到金条，无法「兑换」。\n \n \n \n \n \n \n \n \n                                                  #OFFSET<X:0,Y:-5>##IMAGE1902700019##OFFSET<X:0,Y:-2>#<@Jin_DH *01*「返回」>"
	end
	...
	return "金币「兑换」成功！\n \n \n \n \n#OFFSET<X:0,Y:1>##IMAGE1902700037##OFFSET<X:0,Y:-1>#<@ExchangeGoldEx *01*[继续兑换]>\n \n \n                                                  #OFFSET<X:0,Y:2>##IMAGE1902700019##OFFSET<X:0,Y:-2>#<@Jin_DH *01*「返回」>"
end
--金币金条「兑换」
...
function GenBagEx(npc, player, types)
    ...
    if not lualib:Player_SubGold(player, table_bag[types][4], false, "扣金币：辟岭村仓库包裹打造花费", "天桑村仓库") then
        return "扣除金币失败"
    end
...
function QuestAccomplish1113(npc, player)
    ...
    msg = msg.."#IMAGE1902700017# <@Accomplish1113a *01* 包裹和高级仓库可扩充>\n"
    return msg
end

function Accomplish1113a(npc, player)
    msg = "东西您可是收了！那我就有所求了！\n \n"
    msg = msg.."#IMAGE1902700017# <@Accomplish1113b *01* 呃……请说。>\n"
    return msg
end
