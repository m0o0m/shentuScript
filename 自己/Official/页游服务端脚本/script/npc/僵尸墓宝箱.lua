...
function ExchangeItem(npc, player)
	--开始开启进度条
	if lualib:ProgressBarStart(player, 5000, "僵尸墓宝箱", "progress_complete", "progress_abort", npc) == false then
		return "出错了，请联系GM@2！"
	end
...
function progress_complete(player, npc)		--进度条成功
	...
	if really_group[1] == true then
        ...
        ret = lualib:Player_AddGold(player, gold_value, false, "加金币：打开废弃矿洞宝箱", player)
		if ret == false then
			lualib:SysMsg_SendWarnMsg(player, "系统错误，请联系GM@1！")
			lualib:Npc_Remove(npc)
			return
		else
			lualib:SysMsg_SendPromptMsg(player, "恭喜你，开启废弃矿洞宝箱获得"..gold_value.."金币！")
		end
	end

	--掉落元宝
	if really_group[2] == true then
        ret = lualib:Player_AddIngot(player, item_group[2][1], true, "加元宝：打开废弃矿洞宝箱", player)
		if ret == false then
			lualib:SysMsg_SendWarnMsg(player, "系统错误，请联系GM@2！")
			lualib:Npc_Remove(npc)
			return
		else
			lualib:SysMsg_SendPromptMsg(player, "恭喜你，开启废弃矿洞宝箱获得"..item_group[2][1].."绑定元宝！")
			lualib:SysMsg_SendCenterMsg(1, "玩家"..lualib:Player_GetStrProp(player,  lua_role_user_name).."开启废弃矿洞宝箱获得"..item_group[2][1].."绑定元宝！", "")
		end
	end
		...
		if really_group[i] == true then
			...
			for j = 1, table.getn(item_group[i]) do
				if probability_value >= item_group[i][j][3] and probability_value <= item_group[i][j][4] then
					...
					ret = lualib:Player_GiveItemUnbind(player, item_group[i][j][1], item_group[i][j][2], "给物品：僵尸墓宝箱", player)
					if ret == false then
						...
						if item_group[i][j][5] == 1 then
							lualib:SysMsg_SendCenterMsg(1, "玩家"..lualib:Player_GetStrProp(player,  lua_role_user_name).."开启废弃矿洞宝箱获得"..item_group[i][j][1].."*"..item_group[i][j][2], "")
							lualib:SysMsg_SendBroadcastColor("玩家#COLORCOLOR_BROWN#["..lualib:Player_GetStrProp(player,  lua_role_user_name).."] #COLORCOLOR_YELLOW#开启废弃矿洞宝箱获得#COLORCOLOR_BROWN#"..item_group[i][j][1].."*"..item_group[i][j][2], "", 1, 12)
						end
					break
				end
			end
