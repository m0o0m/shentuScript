...
function key(player, item)
    ...
    local require_item = "高级藏宝图碎片"
    local award_item = "高级藏宝图"
    ...
    if not lualib:Player_DestroyItem(player, require_item, 5, "扣物品：「合成」高级藏宝图", "") then
        return "「合成」失败！\n \n<@leave *01「关闭」>"
    end

    if not lualib:Player_GiveSingleItem(player, award_item, 0, "给物品：「合成」高级藏宝图", "") then
        return "「合成」失败！\n \n<@leave *01*「关闭」>"
    end
