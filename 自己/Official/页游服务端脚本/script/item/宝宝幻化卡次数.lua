...
function main(player, item)
	...
	local baobao = lualib:Player_GetServantList(player)

	local msg = "#OFFSET<X:80,Y:-0>##COLORCOLOR_BROWN#[宝宝幻化卡]#COLOREND#\n"
	...
	msg = msg .. "#IMAGE<ID:1902700033>##COLORCOLOR_BROWN#点击[变]可将你的宝宝幻化成其它外形哦~#COLOREND#\n#OFFSET<X:0,Y:5>#"  
	
	for k, v in pairs(baobao) do
		local bb_name = lualib:Name(v)
		local map_name = lualib:Name(lualib:MapGuid(v))
		msg = msg .. "#IMAGE<ID:1902700015>#".."#OFFSET<X:-22,Y:-1>#<@bianshen".."#"..bb_name.."#"..v.."#"..item.. " *01* 变>".."→#COLORCOLOR_GREENG#["..bb_name.."]#COLOREND# \n"
	end
...
function bianshen(player, bb_name, v, item)
	...
	if hashorse == 1 then
		...
		lualib:AddAvataBuff(v, avatar, "变怪显名", "海魔砸晕", 0)
