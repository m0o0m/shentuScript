...
function key(player, item)
    ...
    local require_item = "封魔印"
    local award_item = "封魔令牌"
    ...
    if not lualib:Player_DestroyItem(player, require_item, 20, "扣物品：「合成」封魔令牌", "") then
        return "「合成」失败！\n \n<@leave *01「关闭」>"
    end

    if not lualib:Player_GiveSingleItem(player, award_item, 0, "给物品：「合成」封魔令牌", "") then
        return "「合成」失败！\n \n<@leave *01*「关闭」>"
    end
