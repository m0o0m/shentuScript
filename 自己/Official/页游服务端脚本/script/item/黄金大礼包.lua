...
local request_slots = 6
	local item_array_man   = {"1.5倍经验卷轴", "逍遥游(100次)" ,"随机传送(500次)" ,"1000绑定元宝", "小喇叭"}
	local item_array_woman = {"1.5倍经验卷轴", "逍遥游(100次)" ,"随机传送(500次)" ,"1000绑定元宝", "小喇叭"}
	local item_count = {3, 1, 1 ,1 ,1}
	...
	local role_gender = lualib:Player_GetIntProp(player_guid,  lua_role_gender)  --获取性别
		if role_gender == 1 then
			for element in pairs(item_array_man) do
				local result = lualib:Player_GiveItemBind(player_guid, item_array_man[element], item_count[element], "给物品：回归礼包", player_guid)
			...
			for element in pairs(item_array_woman) do
				local result = lualib:Player_GiveItemBind(player_guid, item_array_woman[element], item_count[element], "给物品：回归礼包", player_guid)
				if result == false then
				    lualib:SysMsg_SendTriggerMsg(player_guid, "添加道具失败！")
					return false
				end
