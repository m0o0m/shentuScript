...
local request_slots = 8
	local item_array_man   = {"定位石", "时之沙砾", "巫山城传送石", "龙城传送石", "强效金创药包", "强效魔法药包", "绑钱袋(2万)", "新手25级礼包"}
	local item_array_woman = {"定位石", "时之沙砾", "巫山城传送石", "龙城传送石", "强效金创药包", "强效魔法药包", "绑钱袋(2万)", "新手25级礼包"}
	local item_count = {		1, 			2,			 1, 			1,			1,				1,				1,			1}
	...
	local role_gender = lualib:Player_GetIntProp(player_guid,  lua_role_gender)  --获取性别
		if role_gender == 1 then
			for element in pairs(item_array_man) do
				local result = lualib:Player_GiveItemBind(player_guid, item_array_man[element], item_count[element], "给物品：新手卡礼包(20级)", player_guid)
			...
			for element in pairs(item_array_woman) do
				local result = lualib:Player_GiveItemBind(player_guid, item_array_woman[element], item_count[element], "给物品：新手卡礼包(20级)", player_guid)
				if result == false then
				    lualib:SysMsg_SendTriggerMsg(player_guid, "添加道具失败！")
					return false
				end
