...
function on_apply_on_item(player, item, target_item)
    ...
    -- 等级,概率 键值对，概率必须递增！
	--原设定期望为50-60元得到二条，300块得到三条，期望高级符为10元，设定为25%得到两条，5%得到3条
    local rate = {{1,7000}, {2,2500}, {3,500}}
	...
	if not lualib:Player_SubGold(player, subGold, false, "", "") then
		lualib:SysMsg_SendWarnMsg(player, "金币扣除失败")
		return false
	end
