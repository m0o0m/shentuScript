...
function can_accomplish(player, quest_id)
	local tb = {"裂天魔狮", "神兽"}
	for i = 1, #tb do
		if lualib:ItemCountByKey(player, tb[i], true, false, false, 2) > 0 then lualib:SetInt(player, "4026", 1) end		
	end
	
	local a = lualib:GetInt(player, "4026")
	if a ~= 1 then
	return false
	end
	return true
...
function tijiao_ma_renwu(player, item_guid)
	...
	if lualib:Item_IsBind(player, item_guid) == true then
		if lualib:Item_Destroy(player, item_guid, "提交任务删除物品", player) == true then
			lualib:Player_AddGold(player, 1500000, true, "完成任务奖励金币", player)
			lualib:SysWarnMsg(player, "你获得了1500000绑定金币！")
		else
		...
		if lualib:Item_Destroy(player, item_guid, "提交任务删除物品", player) == true then
			lualib:Player_AddGold(player, 1500000, false, "完成任务奖励金币", player)
			lualib:SysWarnMsg(player, "你获得了1500000金币！")
		else
			lualib:NPCTalk(player, "提交任务物品失败！！！")
			return ""
		end
	end
	lualib:DropQuest(player, 4026)
