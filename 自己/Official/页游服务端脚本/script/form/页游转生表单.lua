...
function BuyRepair(player,index)
	...
	if tonumber(index) == 1 then 		
		if yb < 1000 then
			...
			local num = lualib:GetDayInt(player,"高级转生丹次数")
			if num > 19 then
				lualib:SysCenterMsg(0,"今天使用高级转生丹已达20次，不能再使用",player)
				...
				lualib:SetDayInt(player,"高级转生丹次数",num + 1)
				local repair = lualib:GetInt(player,"ReinRepair")
	...
	elseif tonumber(index) == 2 then
		if yb < 9888 then
			...
			local num = lualib:GetDayInt(player,"超级转生丹次数")
			if num > 9 then
				
				lualib:SysCenterMsg(0,"今天使用超级转生丹已达10次，不能再使用",player)
				...
				lualib:SetDayInt(player,"超级转生丹次数",num + 1)
				local repair = lualib:GetInt(player,"ReinRepair")
				lualib:ShowFormWithContent(player,"脚本表单","RoleReinPoint.RepairChange("..repair..");")
			end	
		end	
...
function zhuanshen(player)
	...
	if reinNum >= 7 then
		lualib:ShowFormWithContent(player,"脚本表单","RoleRein.msg_up(\"已经转生到最高等级\");")
		return ""
	end
	...
	if lualib:HasQuest(player, 103) then          --转生等级达到一级时完成131号任务
		local  reinLevel = lualib:GetInt(player,"reinLevel")
		if reinLevel >= 1 then 
			lualib:SetQuestGroupHuntingsCount(player, 103, 444, 1)	
		end
	elseif lualib:HasQuest(player, 105) then          --转生等级达到一级时完成131号任务
		local  reinLevel = lualib:GetInt(player,"reinLevel")
		if reinLevel >= 2 then 
			lualib:SetQuestGroupHuntingsCount(player, 105, 444, 1)	
		end
	elseif lualib:HasQuest(player, 106) then          --转生等级达到一级时完成131号任务
		local  reinLevel = lualib:GetInt(player,"reinLevel")
		if reinLevel >= 3 then 
