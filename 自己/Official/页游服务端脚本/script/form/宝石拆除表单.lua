...
function loadEquipHoles(player, guid)
	...
	lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.initHoles(" .. serialize(holesTb) .. "," .. enableHolesNum .. ");")
	return ""        
end
		...
		for j = i + 1, 4 do
			if stoneNameTb[i] ~= "" and stoneNameTb[j] ~= "" then
				...
				if newStr1 == newStr2 then
					lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.attentionMsg(2,\"一件装备只能镶嵌一个同种类的宝石\");")
					return ""
				end
			end
		end
	...
	for i = 1, 4 do
		...
		if hole == "2" and tb[i] ~= "" then
			...
			if not lualib:DelItem(player, keyname,1,2, "宝石镶嵌消耗", player) then 
				lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.attentionMsg(2,\"没有这个宝石\");")
				return ""
			end
...
lualib:ShowFormWithContent(player,"脚本表单","WndAddEffect(nil,\"CrystalSystem,DismantleCrystal\",3006600000,74,-27,200,1);")
				lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.attentionMsg(2,\"镶嵌成功\");")
			end
		end
	end
...
function unsealHole(player, equipGuid, holeNum)
			...
			for i = 1, #tb do
				...
				if key == "破封印" then
					if lualib:Item_Destroy(player, tb[i], "宝石孔解除封印", player) then
						...
						lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.unsealHole(" .. holeNum .. ");DismantleCrystal.attentionMsg(4,\"解封成功\");")
						lualib:Item_NotifyUpdate(player, equipGuid)
						return ""
					end
				end
	...
	lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.attentionMsg(4,\"缺少破封印\");")
	return ""
end
...
function getSubGold(player, equipGuid, indexNum)
	if tonumber(indexNum) ~= 0 then
		...
		if keyName ~= "0" and keyName ~= "1" and keyName ~= "2" then
			...
			lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.takeOffNeedGold(" .. costMoney .. ");")
		end
	end
	return ""
end
		...
		if keyName ~= "0" and keyName ~= "1" and keyName ~= "2" then
			...
			--	lualib:SysPromptMsg(player,costMoney..":"..bindGold..":"..gold)
				if costMoney <= bindGold + gold then
					lualib:SubBindGold(player, costMoney, "摘除宝石:优先扣除绑金", player)
					if costMoney > bindGold then
						costMoney = costMoney - bindGold
						lualib:SubGold(player, costMoney, "摘除宝石:绑金不足扣金币", player)
					end
				else
					lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.attentionMsg(2,\"金币不足\");")
					return ""
				end
				...
				if lualib:SetStr(equipGuid, "EquipHole" .. indexNum, "2") then
					if lualib:AddItem(player, keyName, 1, false, "拆除镶嵌宝石获得", player) then
						lualib:Item_NotifyUpdate(player, equipGuid)
						lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.takeOffFinish();")
					end
				end
			else
				lualib:ShowFormWithContent(player, "脚本表单", "DismantleCrystal.attentionMsg(2,\"包裹空间不足\");")
				return ""
			end
		end
	end
