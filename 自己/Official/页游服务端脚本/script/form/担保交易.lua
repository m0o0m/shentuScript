...
function main(player, types, price, num)
	...
	local function gold()
		...
		if not lualib:PostURL(url, webStr, "callback", webStr, 5) then
			...
			return "因为网络原因，您商品担保交易提交失败，返回你的物品，请稍后再试！"
		else
			lualib:LogDetail(playerName, "担保交易", "担保交易", 0, 0, "订单："..player.."#"..types.."#"..price.."#"..num, "", "")
			lualib:SysMsg_SendTriggerMsg(player, "恭喜您，你的商品已成功上架！")
		end
		return ""
	end
...
function main2(player, price, param)
	...
	for i = 1, #t do
		...
		if i == #t then
			itemJsonStr = itemJsonStr..itemJson
		else
			itemJsonStr = itemJsonStr..itemJson.."#"
		end
	end
	...
	local url = "http://api.hzyotoy.com:8800/api/creategameorder.do"
	if not lualib:PostURL(url, webStr, "callback", webStr, 5) then
		local item_t = string.split(itemJsonStr, "#")
		
		for i = 1, #item_t do
			if not lualib:MailJsonItem("担保交易系统", playerName, "因为网络原因，您商品担保交易提交失败，返回你的物品，请稍后再试！", 0, 0, item_t[i]) then
				lualib:LogDetail(playerName, "担保交易", "担保交易", 0, 0, "webStr", "", "")
			end
		end
		...
		lualib:LogDetail(playerName, "担保交易", "担保交易", 0, 0, "订单："..player.."#"..price.."#"..param, "", "")
		lualib:SysMsg_SendTriggerMsg(player, "恭喜您，你的商品已成功上架！")
	end
	return ""
end
...
function main3(...)
	...
	local function gold()
		if not lualib:Mail("担保交易系统", playerName, "你所购买的订单号："..id.."商品已到达，请查收！", tonumber(t[4]), 0, {}) then
	...
	local function Ingot()
		if not lualib:Mail("担保交易系统", playerName, "你所购买的订单号："..id.."商品已到达，请查收！", 0, tonumber(t[4]), {}) then
			lualib:LogDetail(playerName, "担保交易失败", "担保交易失败", 0, 0, "订单："..id.."#"..types.."#"..param, "", "")
		else
			lualib:LogDetail(playerName, "担保交易成功", "担保交易成功", 0, 0, "订单："..id.."#"..types.."#"..param, "", "")
		end
...
function item(...)
	...
	for i = 3, #t do
		if not lualib:MailJsonItem("担保交易系统", name, "你所购买的订单号："..id.."商品已到达，请查收！", 0, 0, t[i]) then
...
function callback(param, errcode, web_result)
	...
	if web_result ~= "OK" then
		if t[7] == "1" then
			if not lualib:Mail("系统", t[6], "因为网络原因，您商品担保交易提交失败，返回你的物品，请稍后再试！", tonumber(t[9]), 0, {}) then
				lualib:LogDetail(name, "担保交易", "担保交易", 0, 0, param, "")
			end
		elseif t[7] == "2" then
			if not lualib:Mail("系统", t[6], "因为网络原因，您商品担保交易提交失败，返回你的物品，请稍后再试！", tonumber(t[9]), 0, {}) then
				lualib:LogDetail(name, "担保交易", "担保交易", 0, 0, param, "")
		...
		elseif t[7] == "3" then
			if #t >= 9 then
				for i = 9, #t do
					if not lualib:MailJsonItem("担保交易系统", t[6], "因为网络原因，您商品担保交易提交失败，返回你的物品，请稍后再试！", 0, 0, t[i]) then
						lualib:LogDetail(name, "担保交易", "担保交易", 0, 0, param, "")
					end
				end
			end
