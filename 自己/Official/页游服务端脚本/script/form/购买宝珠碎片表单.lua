...
function buy(player,index)
	...
	if index_num == 1 then 
		...
		if lualib:Player_IsIngotEnough(player, 1000, false) == false then
			...
			--lualib:ShowFormWithContent(player,"脚本表单","ShenLu_buy3.msgbox()")
			return ""
		else
			if lualib:Player_SubIngot(player, 1000, false, "快捷购买成就", player) == false then
				return "扣除元宝失败"
			else
				chip = chip + 40
				if lualib:SetInt(player,"baoshi_chip",chip) == false  then
				   return "宝石结晶(中)使用失败"
					...
					lualib:SysTipsMsg(player,"宝石碎片增加40")
		...
		if index_num == 2 then 
			...
			if lualib:Player_IsIngotEnough(player, 9888, false) == false then
				...
				if lualib:Player_SubIngot(player, 9888, false, "快捷购买成就", player) == false then
					return "扣除元宝失败！" 
				else
					chip = chip + 400
					if lualib:SetInt(player,"baoshi_chip",chip) == false  then
					   return "宝石结晶（大）使用失败"
						...
						lualib:SysTipsMsg(player,"宝石碎片增加400")
						AddKaiFuMagicOnBaoShiLevelUp(player)
					end
					--********************************--
					local indexx = lualib:GetDBNum("consumeTerm")
	...
	lualib:ShowFormWithContent(player,"脚本表单","ShenLu_buy3.updata();FurnaceWnd.chipupdata("..chip..")")
