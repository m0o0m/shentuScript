...
local npc_keyname =
{
	...
	["dlc_keyname"] = {"运货员1", "运货员2", "挑水夫1", "挑水夫2", "小贩5", "运货员3", "运货员4", "伐木工人1"},
}
...
local lj =
{
	...
	["dlc_keyname"] =
	{
		...
		["挑水夫1"] = {313 ,270, 297 ,281, 308 ,293, 304 ,310, 325 ,279},
		["挑水夫2"] = {350 ,310, 338 ,311, 337 ,330, 357 ,340, 365 ,316, 351 ,301, 351 ,284},
...
local str =
{
	...
	["dlc_keyname"] =
	{
		...
		["挑水夫1"] = {"病从口入，张开你的嘴！", "这药可是从十万八千里的高山上采下来的，你自己说，我卖贵一点怎么了？", "黄金有价药无价，懂吗？"},
		["挑水夫2"] = {"假药卖不得！", "这年头假药的销量比真药还好，真药都卖不出去，我……？", "好了，别说了，爱买不买，反正有病的不是我！"},
...
function main(monster, player)
	lualib:SysMsg_SendRoleTalk(monster, "纯洁的人做纯洁的事挑纯洁的水！你懂的……")
	local msg = "#COLORCOLOR_BROWN#━━━━━━━━━━━━━━━#COLOREND##COLORCOLOR_SKYBLUE#挑水夫#COLOREND##COLORCOLOR_BROWN#━━━━━━━━━━━━━━━#COLOREND#\n"
	msg = msg .. "过往山岭、千帆阅尽！我已从中领悟神力！只要你能给我#COLORCOLOR_GREENG#珍珠、#COLORCOLOR_RED#生生草、#COLORCOLOR_YELLOW#幽暗之花#COLOREND#、我便可给你一些特殊功能的神丹！你有这些材料的话，可放在#COLORCOLOR_YELLOW#包裹第1格#COLOREND#，到我这里来炼化，对于费用嘛，我分文不收！\n"
	msg = msg .. "#OFFSET<X:0,Y:10>##IMAGE1902700037#<@zhiyao#"..player.. "#*01* [炼丹]>\n"
...
function zhiyao(monster, player)
	...
	if zb_keyname1 == cl[1]  or zb_keyname1 == cl[2] or zb_keyname1 == cl[3] then
		lualib:SysMsg_SendMsgDlg(player, 1, "#COLORCOLOR_WHITE#每次炼丹至少需要20个材料，你确定要炼丹吗？\n \n#COLOREND##BUTTON0#确定#BUTTONEND#", 100, "on_msg_dlg_ack", "")
	else
		lualib:MsgBox(player, "你的包裹第1格的物品不是我所需要的材料，不能炼丹！")
	end
...
function on_msg_dlg_ack(dlg_id, player, button_id, param)
	...
	local yaotb_10 = {"怒攻神丹", "怒法神丹", "怒道神丹", "力盾神丹", "法盾神丹", "怒血神丹", "怒魔神丹", "恢复神丹", "疾风神丹", "疾驰神丹"}
	...
	if zb_keyname1 == cl[1]  or zb_keyname1 == cl[2] or zb_keyname1 == cl[3] then
		...
		if dd_num < 20 then
			lualib:MsgBox(player, "你的材料少于20个，不能炼丹！")
			return
		end
		
		if not lualib:DelItem(player, zb_keyname1, 20, 2, "挑水夫炼丹", player) then
			lualib:MsgBox(player, "很遗憾，炼丹失败！")
		end
	...
	if not lualib:AddItem(player, yaotb_10[lualib:GenRandom(1, 10)], 1, false, "挑水夫炼丹", player) then
		lualib:MsgBox(player, "很遗憾，炼丹失败！")
	end
	lualib:SysMsg_SendCenterMsg(1, "["..player.."]→→在[挑水夫]处用海底采集物炼出了一颗威力奇特的神丹！", "")
	lualib:MsgBox(player, "炼丹成功！请查看包裹！")
