...
function main(monster, player)
	lualib:SysMsg_SendRoleTalk(monster, "医行于德，康复于心！")
	local msg = "#COLORCOLOR_BROWN#━━━━━━━━━━━━━━━#COLOREND##COLORCOLOR_SKYBLUE#郎中#COLOREND##COLORCOLOR_BROWN#━━━━━━━━━━━━━━━#COLOREND#\n"
	msg = msg .. "妙手回春，救死扶伤！我可将#COLORCOLOR_GREENG#珍珠、#COLORCOLOR_RED#生生草、#COLORCOLOR_YELLOW#幽暗之花#COLOREND#、制造成灵丹妙药！你有这些材料的话，可放在#COLORCOLOR_YELLOW#包裹第1格#COLOREND#，到我这里来制作，对于费用嘛，我分文不收！\n"
	msg = msg .. "#OFFSET<X:0,Y:10>##IMAGE1902700037#<@zhiyao#"..player.. "#*01* [制药]>\n"
...
function zhiyao(monster, player)
	...
	if zb_keyname1 == cl[1]  or zb_keyname1 == cl[2] or zb_keyname1 == cl[3] then
		lualib:SysMsg_SendMsgDlg(player, 1, "#COLORCOLOR_WHITE#每次制药至少需要30个材料，你确定要制药吗？\n \n#COLOREND##BUTTON0#确定#BUTTONEND#", 100, "on_msg_dlg_ack", "")
	else
		lualib:MsgBox(player, "你的包裹第1格的物品不是我所需要的材料，不能制药！")
	end
...
function on_msg_dlg_ack(dlg_id, player, button_id, param)
	...
	local yaotb_13 = {"小特制金创药", "中特制金创药", "大特制金创药", "小特制魔法药", "中特制魔法药", "大特制魔法药", "回阳水", "强效回阳水", "万年雪霜", "疗伤药", "小特制疗伤药", "中特制疗伤药", "大特制疗伤药"}
	...
	if zb_keyname1 == cl[1]  or zb_keyname1 == cl[2] or zb_keyname1 == cl[3] then
		...
		if dd_num < 30 then
			lualib:MsgBox(player, "你的材料少于30个，不能制药！")
			return
		end
		
		if not lualib:DelItem(player, zb_keyname1, 10, 2, "郎中制药", player) then
			lualib:MsgBox(player, "很遗憾，制药失败！")
		end
	...
	if not lualib:AddItem(player, yaotb_13[lualib:GenRandom(1, 13)], 1, false, "郎中制药", player) then
		lualib:MsgBox(player, "很遗憾，制药失败！")
	end
	lualib:SysMsg_SendCenterMsg(1, "["..player.."]→→在[郎中]处用海底采集物制造了一瓶疗效非凡的药剂！", "")
	lualib:MsgBox(player, "制药成功！请查看包裹！")
