...
function main(monster, player)
	lualib:SysMsg_SendRoleTalk(monster, "学海无涯苦作舟，书山有路勤为径！")
	local msg = "#COLORCOLOR_BROWN#━━━━━━━━━━━━━━━#COLOREND##COLORCOLOR_SKYBLUE#学者#COLOREND##COLORCOLOR_BROWN#━━━━━━━━━━━━━━━#COLOREND#\n"
	msg = msg .. "谈不上上知天文、下知地理，但也算是满腹经纶！只要你能在包裹第1格给我#COLORCOLOR_GREENG#终极技能残页#COLOREND#，我就能给你这世界上#COLORCOLOR_RED#所有书籍#COLOREND#，包括一些#COLORCOLOR_GREENG#特殊的书籍#COLOREND#，如：#COLORCOLOR_RED#落马术#COLOREND#等……当然，这要看你给我的残页数量了，你的心意越大，我给你的书可能就越好喔~\n \n"
	msg = msg .. "#OFFSET<X:0,Y:10>##IMAGE1902700037#<@huanshu#"..player.. "#*01* [残页换书]>\n"
...
function huanshu(monster, player)
	...
	local item_site = {item_site1}
	

	if zb_keyname1 ~= "终极技能残页" then
			lualib:MsgBox(player, "你的包裹第1格的物品不是终极技能残页，文明人不可撒谎哦！")
	else
		lualib:SysMsg_SendInputDlg(player, 1, "数量越多，换到的书可能越好哦！", 45, 14, "on_msg_dlg_ack", monster)
	end
	--lualib:SysMsg_SendMsgDlg(player, 1, "#COLORCOLOR_WHITE#你确定要跟我换吗？换好换坏可是看我心情的，你可别骂娘啊！\n \n#COLORCOLOR_GREENG#小提示：数量越多，换到的书可能越好哦！#COLOREND##BUTTON0#确定#BUTTONEND#", 100, "on_msg_dlg_ack", "")
...
function on_msg_dlg_ack(dlg_id, player, num, param)
	local num = tonumber(num)
	local msg = ""
	if num == nil or num == "" then
		msg = "你的输入错误，请输入正整数！"
		lualib:MsgBox(player, msg)
		return true
	...
	local yd_22 = {"剑术心法",	"炎球术",	"退拒之环",	"夺心诱惑",	"冰箭术",	"瞬息灵动",	"攻心斩",	"治疗咒",	"明镜止水",	"蛊毒咒",	"神魂火符",	"地府之唤",	"隐形咒",	"风刺剑法",	"炎爆术",	"天雷术",	"死亡射线",	"烈焰火海",	"集体隐形咒",	"地煞镇魔咒",	"天罡护体咒",	"锁妖咒"}

	local ed_9 = {"月弧弯刀",	"蛮力冲锋",	"烈焰斩",	"雷霆极光",	"流光护盾",	"杀生术",	"冰爆漩涡",	"回春咒",	"天庭之唤"}
	...
	local dd_num = lualib:Item_GetAmount(player, item_site1)
	
	if dd_num < num then
		lualib:MsgBox(player, "你的残页不足！")
		return
	end


	if not lualib:DelItem(player, zb_keyname1, num, 2, "学者兑换书籍", player) then
		lualib:MsgBox(player, "很遗憾，换取失败！")
	end


		
	if num >= 1 and num < 3 then
		if not lualib:AddItem(player, yd_22[lualib:GenRandom(1, #yd_22)], 1, false, "学者兑换书籍", player) then
			lualib:MsgBox(player, "很遗憾，换取失败！")
		end
		lualib:SysMsg_SendCenterMsg(1, "["..player.."]→→在[学者]处用终极技能残页换取到了一本神秘的技能书！", "")
		lualib:MsgBox(player, "换取成功！请查看包裹！")
	elseif num >= 3 and num < 5 then
		if not lualib:AddItem(player, ed_9[lualib:GenRandom(1, #ed_9)], 1, false, "学者兑换书籍", player) then
			lualib:MsgBox(player, "很遗憾，换取失败！")
		end
		lualib:SysMsg_SendCenterMsg(1, "["..player.."]→→在[学者]处用终极技能残页换取到了一本神秘的技能书！", "")
		lualib:MsgBox(player, "换取成功！请查看包裹！")
	elseif num >= 5 and num < 10 then
		if not lualib:AddItem(player, sd_12[lualib:GenRandom(1, #sd_12)], 1, false, "学者兑换书籍", player) then
			lualib:MsgBox(player, "很遗憾，换取失败！")
		end
		lualib:SysMsg_SendCenterMsg(1, "["..player.."]→→在[学者]处用终极技能残页换取到了一本神秘的技能书！", "")
		lualib:MsgBox(player, "换取成功！请查看包裹！")	
	elseif num >= 10 then
		if not lualib:AddItem(player, sid_14[lualib:GenRandom(1, #sid_14)], 1, false, "学者兑换书籍", player) then
			lualib:MsgBox(player, "很遗憾，换取失败！")
		end
		lualib:SysMsg_SendCenterMsg(1, "["..player.."]→→在[学者]处用终极技能残页换取到了一本神秘的技能书！", "")
		lualib:MsgBox(player, "换取成功！请查看包裹！")
