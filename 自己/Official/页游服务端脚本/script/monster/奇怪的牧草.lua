...
function catch_complete(player, params)		--进度条成功
   ...
   if not lualib:Monster_Remove(params) then
        lualib:SysPromptMsg(player, lualib:Name(params).."不存在，采集失败！")
        return
    end
	
	lualib:AddTimerEx(map, 111, 10000, 1, "caijireborn", "")
...
function caijireborn(map)
	
	lualib:Map_GenMonster(map, re_x, re_y, 100, 4, target_info1[re_keyname], 1, true)
