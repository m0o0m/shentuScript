...
function on_create(monster)
	lualib:SetInt("0","boss_shuaxin4",0)
...
function on_trigger_post_die(actor, killer)
	...
	if lualib:KeyName(lualib:MapGuid(actor)) == "邪庙boss" then
		local killername = lualib:Name(killer)
		lualib:SetInt("0","boss_shuaxin4",90)
		lualib:SetStr("0","boss_killer4",killername)
		lualib:AddTimerEx("0",304,60000,1,"chongshengshijian","")
	end
	
	local playerlist = lualib:Player_GetTeamList(killer)
	for i = 1, #playerlist do
		if lualib:KeyName(lualib:MapGuid(actor)) == "邪庙boss" then
			lualib:OnGloryTrigger(playerlist[i], 2, "", 1, "", "血禅师")
		end
	end
end
...
function chongshengshijian()
	if lualib:GetInt("0","boss_shuaxin4") > 0 then
		lualib:SetInt("0","boss_shuaxin4",lualib:GetInt("0","boss_shuaxin4")-1)
		lualib:AddTimerEx("0",304,60000,1,"chongshengshijian","")
	end
end
