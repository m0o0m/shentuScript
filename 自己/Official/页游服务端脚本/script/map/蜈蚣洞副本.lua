...
function on_enter_map(player) 
	--lualib:ShowFormWithContent(player, "form文件表单", "Onhook")
	return ""
end


function on_pre_monster_die(monster, sPlayerID)
...
function on_post_monster_die(monster, sPlayerID)
	...
	if monsterCount <= 0  then
		...
		lualib:Debug("src = "..sMapKeyName)
		lualib:Debug("dst = "..dMapKeyName)
		lualib:Map_AddJumpPointEx(sMapID, destMap, 37, 34, 2, 161, 200, 3) -- 加入动态跳转点
		local jump_content = sMapKeyName.."#"..dMapKeyName.."#".."37".."#".."34".."#".."2"
		lualib:ShowFormWithContent(sPlayerID,"增加动态跳转点表单",jump_content)
		lualib:RunClientScript(sMapID, "mapeffect", "texiao", "100000009".."#".."37".."#".."34".."#0#2000") --加入场景特效 , 需要在离开副本时删除
...
function oncall( sMapID , iTimerID , sPlayerID )
	...
	elseif thissecond <= 0 then 
		lualib:Player_MapMoveXY( sPlayerID , "巫山城" , 203 , 245 , 3 )
	end
end

function on_leave_map( sPlayerID )
	
	--lualib:ShowFormWithContent(sPlayerID,"脚本表单","Onhook.Closed()")
	lualib:ForceQuestReady( sPlayerID , 37 )
	local sMapID = lualib:MapGuid( sPlayerID )
	lualib:RunClientScript(sMapID, "mapeffect", "del_texiao", "100000009".."#".."11".."#".."56")
