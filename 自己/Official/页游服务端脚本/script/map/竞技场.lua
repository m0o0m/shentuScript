...
function on_trigger_enter_map(player)
	...
	msg = msg .."#OFFSET<X:5,Y:0>##COLORCOLOR_BROWN#1.打赢对手，就能赢得对手的元宝！\n \n#COLOREND#"
	msg = msg .."#OFFSET<X:5,Y:0>##COLORCOLOR_BROWN#2.奖金将会通过邮件的形式发送给你!\n \n#COLOREND#"
	msg = msg .."#OFFSET<X:5,Y:0>##COLORCOLOR_BROWN#3.想赢谁的元宝，就把谁叫进来吧！你行的，你可以的！^_^#COLOREND#"
...
function on_map_destroy(map)
	...
	if xinxi ~= "" then
		...
		if types == 3 then
				lualib:MailToPlayer("竞技场管理员", xinxi_tbale[1], "战斗结束，本场战斗平局，返回您押注90%的彩金", 0, tonumber(xinxi_tbale[2])*90/100, {})
				lualib:MailToPlayer("竞技场管理员", xinxi_tbale[3], "战斗结束，本场战斗平局，返回您押注90%的彩金", 0, tonumber(xinxi_tbale[2])*90/100, {})
				for i = 1, #zhandou do
			...
			elseif types ==11 then
				lualib:MailToPlayer("竞技场管理员", xinxi_tbale[1], "战斗结束，本场战斗平局，返回您押注90%的彩金", 0, tonumber(xinxi_tbale[6])*90/100, {})
				lualib:MailToPlayer("竞技场管理员", xinxi_tbale[7], "战斗结束，本场战斗平局，返回您押注90%的彩金", 0, tonumber(xinxi_tbale[6])*90/100, {})
				for i = 1, #zhandou do
					for j = 1, #zhandou[i] do
						local value = lualib:IO_GetCustomVarStr(zhandou[i][j][2])
						if value == xinxi then
			...
			elseif types == 21 then
				lualib:MailToPlayer("竞技场管理员", xinxi_tbale[1], "战斗结束，本场战斗平局，返回您押注90%的彩金！", 0, tonumber(xinxi_tbale[11])*90/100, {})
				lualib:MailToPlayer("竞技场管理员", xinxi_tbale[12], "战斗结束，本场战斗平局，返回您押注90%的彩金", 0, tonumber(xinxi_tbale[11])*90/100, {})
				for i = 1, #zhandou do
					for j = 1, #zhandou[i] do
						local value = lualib:IO_GetCustomVarStr(zhandou[i][j][2])
						if value == xinxi then
...
function panding(map, id)
	...
	if a_renshu > 0 and b_renshu <= 0 then
		...
		lualib:MailToPlayer("竞技场管理员", xinxi_tbale[1], "战斗结束，你赢得了战斗！", 0, tonumber(xinxi_tbale[2])*1.8, {})
		for i = 1, #zhandou do
			for j = 1, #zhandou[i] do
				local value = lualib:IO_GetCustomVarStr(zhandou[i][j][2])
				if value == xinxi then
	...
	elseif b_renshu > 0 and a_renshu <= 0 then
		if types == 3 then
			lualib:MailToPlayer("竞技场管理员", xinxi_tbale[3], "战斗结束，你赢得了战斗！", 0, tonumber(xinxi_tbale[2])*180/100, {})
		...
		elseif types == 11 then
			lualib:MailToPlayer("竞技场管理员", xinxi_tbale[7], "战斗结束，你赢得了战斗！", 0, tonumber(xinxi_tbale[6])*180/100, {})
		...
		elseif types == 21 then
			lualib:MailToPlayer("竞技场管理员", xinxi_tbale[12], "战斗结束，你赢得了战斗！", 0, tonumber(xinxi_tbale[11])*180/100, {})
