...
function on_enter_map(player) 
	...
	local tips2 = lualib:GetInt(player,"Tzboss_tips2") 
	if tonumber(tips2) == 0 then 
		--lualib:ShowFormWithContent(player,"form文件表单","Onhook")
		lualib:ShowFormWithContent(player, "脚本表单", "GameMainBar.TweenDown();")
		lualib:SetInt(player,"Tzboss_tips2",1)
	end
	return ""
end
...
function on_post_monster_die( monster , sPlayerID )
	...
	if monster_count == 0 then
		...
		lualib:SetDayInt(sPlayerID, "qianwang5",1)
		lualib:SetInt(sPlayerID, "qianwang5",1)
		lualib:NotifyVar(sPlayerID, "qianwang5")
		lualib:AddTimerEx(sPlayerID,868686,15000,1,"on_time","") 
		
		local num = lualib:GetInt(sPlayerID, "天荒教主副本one") 
		if num == 0 then 
			lualib:SetInt(sPlayerID, "天荒教主副本one",1) 
			local x = lualib:X(monster)
			local y = lualib:Y(monster)
			local item_tab = {"战将手镯","法灵手镯","散人手镯"}
