...
function on_enter_map(player)
	lualib:ShowFormWithContent(player, "form文件表单", "DailyFbWnd#\"1级宝石箱\"#2#600000#31#\"1级宝石箱\"")
...
function GetReward_6(sMapID, player)
	local iMonster_count = lualib:Map_GetMonsterCount( sMapID , "" , true , true )
	if iMonster_count <= 0 then
		if not (lualib:BagFree(player, true, false, false) > 0) then
			if lualib:MailToPlayer("雪域秘境", player , "您选择双倍副本奖励，请注意查收" , 0 , 0 , { "1级宝石箱" , 2 , 1 } ) then
				if lualib:Player_AddExp(player ,600000 , "雪域秘境" ,"system") then
					onLeave(sMapID, player)
				end
			end
		else	
			if lualib:AddItem(player , "1级宝石箱" , 2 , true , "副本镇魔圣殿奖励", "每日副本系统") then
				if lualib:Player_AddExp(player ,600000 ,"雪域秘境" ,"system") then
					onLeave(sMapID, player)
				end
			end
...
function GetDoubleReward_6(sMapID, player)
	local iMonster_count = lualib:Map_GetMonsterCount( sMapID , "" , true , true )
	if iMonster_count <= 0 then
		if not (lualib:GetIngot(player) >= 38) then
			lualib:ShowFormWithContent(player, "脚本表单", "DailyFbWnd.MsgBox()")
		else
			if lualib:SubIngot(player, 38, "每日副本", "系统") then
				if not (lualib:BagFree(player, true, false, false) > 0) then
					if lualib:MailToPlayer("雪域秘境", player , "您选择双倍副本奖励，请注意查收" , 0 , 0 , { "1级宝石箱" , 4 , 1 } ) then
						if lualib:Player_AddExp(player ,1200000 , "雪域秘境" ,"system") then
							onLeave(sMapID, player)
						end
					end
				else
					if lualib:AddItem(player , "1级宝石箱" , 4 , true , "副本镇魔圣殿双倍奖励", "每日副本系统") then
						if lualib:Player_AddExp(player ,1200000 , "雪域秘境" ,"system") then
							onLeave(sMapID, player)
						end
					end
