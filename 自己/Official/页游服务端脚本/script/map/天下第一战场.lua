...
--[[    lualib:Print("天下第一战场：进入地图回调！")
	...
	msg = msg .."#OFFSET<X:5,Y:0>##COLORCOLOR_BROWN#1.天下第一战场内，必须只有一个人才能获得胜利！\n \n#COLOREND#"
	msg = msg .."#OFFSET<X:5,Y:0>##COLORCOLOR_BROWN#2.只剩你一人时，在场内等一会，将会自动获得第一，然后出去到武馆教练领取奖励!\n \n#COLOREND#"
	msg = msg .."#OFFSET<X:5,Y:0>##COLORCOLOR_BROWN#3.你是最强的，我看好你哟^_^#COLOREND#"
...
--[[    local job_table = {"战士", "法师", "道士", ""}
    ...
    if player_count ~= 1 then
		...
		lualib:SysMsg_SendBroadcastColor("天下第一"..job_table[wday].."争霸赛结束，玩家无胜负！", "", 1, 12)
		SetMaster("", wday)
    else
        ...
        lualib:SysMsg_SendTopMsg(3, "恭喜"..name.."力挫群雄，最终赢得了天下第一"..job_table[wday].."争霸赛的冠军宝座，篆刻雕象，名留青史！")
        lualib:SysMsg_SendCenterMsg(1, "恭喜"..name.."力挫群雄，最终赢得了天下第一"..job_table[wday].."争霸赛的冠军宝座，篆刻雕象，名留青史！", "")
		lualib:SysMsg_SendBroadcastColor("恭喜#COLORCOLOR_BROWN#["..name.."] #COLORCOLOR_YELLOW#力挫群雄，最终赢得了天下第一"..job_table[wday].."争霸赛的冠军宝座，篆刻雕象，名留青史！", "", 1, 12)
    end--]]
end
