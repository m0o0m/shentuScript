...
function on_map_create(map)
	...
	if not lualib:AddTimerEx(map, 3102, 14400000, -1, "shuazuoqi", "") then
...
function SubPlayerTime(map, timer_id)
	...
	for k1, v1 in pairs(playerList) do
		if lualib:Player_GetCustomVarInt(v1, "time") <= 0 then
			...
			lualib:Player_SetCustomVarInt(v1, "time", lualib:Player_GetCustomVarInt(v1, "time") - 5)
			if lualib:Player_GetCustomVarInt(v1, "time") <= 0 then
...
function on_trigger_enter_map(player)
    ...
    lualib:SysMsg_SendPromptMsg(player, "异兽栏的时间流逝速度比初级木人巷快四秒，请珍惜时间！^_^")
	...
	msg = msg .."#OFFSET<X:5,Y:0>##COLORCOLOR_BROWN#3.异兽栏的时间流逝速度比初级木人巷快四秒，请珍惜时间！^_^#COLOREND#"
...
function shuazuoqi(map, timeid)
    local horse_num = lualib:Map_GetMonsterCount(map, "稀有坐骑", true, true)
	
	if horse_num >= 1 then
		return 
	else
	--[[
		lualib:Map_GenMonster(map, 49, 61, 5, 8, "稀有坐骑", 1, false)
		local s = "[藏宝阁]的[异兽栏]里传出一声奇怪的吼叫~"
		local s1 = "[藏宝阁]的[异兽栏]里传出一声奇怪的吼叫~"
