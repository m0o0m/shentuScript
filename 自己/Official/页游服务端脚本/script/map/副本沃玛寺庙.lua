...
function GetReward_1(sMapID, player)
	local iMonster_count = lualib:Map_GetMonsterCount( sMapID , "" , true , true )
	if iMonster_count <= 0 then
		if not (lualib:BagFree(player, true, false, false) > 0) then
			if lualib:MailToPlayer("天荒古阵", player , "您选择双倍副本奖励，请注意查收" , 0 , 0 , { "魂珠碎片小" , 2 , 1 } ) then
				if lualib:Player_AddExp(player ,1500000 , "天荒古阵" ,"system") then
					onLeave(sMapID, player)
				end
			end
		else	
			if lualib:AddItem(player , "魂珠碎片小" , 2 , true , "副本天荒古阵奖励", "每日副本系统") then
				if lualib:Player_AddExp(player ,1500000 ,"天荒古阵" ,"system") then
					onLeave(sMapID, player)
				end
			end
...
function GetDoubleReward_1(sMapID, player)
	local iMonster_count = lualib:Map_GetMonsterCount( sMapID , "" , true , true )
	if iMonster_count <= 0 then
		if not (lualib:GetIngot(player) >= 98) then
			...
			if lualib:SubIngot(player, 98, "每日副本", "系统") then
				if not (lualib:BagFree(player, true, false, false) > 0) then
					if lualib:MailToPlayer("天荒古阵", player , "您选择双倍副本奖励，请注意查收" , 0 , 0 , { "魂珠碎片小" , 4 , 1 } ) then
						if lualib:Player_AddExp(player ,3000000 , "天荒古阵" ,"system") then
							onLeave(sMapID, player)
						end
					end
				else
					if lualib:AddItem(player , "魂珠碎片小" , 4 , true , "副本天荒古阵双倍奖励", "每日副本系统") then
						if lualib:Player_AddExp(player ,3000000 , "天荒古阵" ,"system") then
							onLeave(sMapID, player)
						end
					end
