LuaUI["附魔石吸收"].OnInit = function(handle, param, x, y)
	...
	local description = "1.放入无属性附魔石和装备\n2.吸收属性后装备消失"
	RichEditSetProp(handle, "Description", nil, false, false, "system", MakeARGB(255, 156, 150, 33), description)
	EditSetProp(handle, "GUI_9", nil, nil, false, true, MakeARGB(255, 215, 170, 117), false, "附魔石")
	EditSetProp(handle, "GUI_10", nil, nil, false, true, MakeARGB(255, 215, 170, 117), false, "装备")
...
LuaUI["附魔石吸收"].ItemMoveIn = function()
	...
	if handle ~= 0 and as3.tolua(UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ITEMID)) then
		...
		if not as3.tolua(UI:Lua_GetItemTemplatePropByID(itemid, ITEM_PROP_SUBTYPE)) then return end
		local item_subtype = as3.tolua(LuaRet)
		if item_type == ITEM_TYPE_MATERIAL and item_subtype == 20 then
			RDItemCtrlFillGUIData(handle, "Item0", _GUIDataHandle)
		elseif item_type == ITEM_TYPE_EQUIP and item_subtype ~= EQUIP_JUJU and item_subtype ~= EQUIP_MOUNT then
...
LuaUI["附魔石吸收"].OnResult = function()
	...
	RDItemCtrlClearGUIData(nil, "附魔石吸收,Item1")
end
