...
function SetWindowPosAt(XPerct,YPerct)
	...
	UI:Lua_WndGetSizeM()
	local _W = as3.tolua(LuaRet[1])
	local _H = as3.tolua(LuaRet[2])
...
function GetTipItemGUIDataPropByType(Type)
	...
	return as3.tolua(UI:Lua_GetItemGUIDataPropByType(handle,Type))
end

--获取物品格属性
function GetWndItemGUIDataPropByType(Type)
	if as3.tolua(UI:Lua_WndGetItemGUIDataHandle()) then
		return as3.tolua(UI:Lua_GetItemGUIDataPropByType(as3.tolua(LuaRet),Type))
	end
	return false
end
...
function SetWndItemGUIDataPropByType(Type,Value)
	if as3.tolua(UI:Lua_WndGetItemGUIDataHandle()) then
		LuaArg = Value
		CL:SetLuaArg(LuaArg);
		return as3.tolua(UI:Lua_SetItemGUIDataPropByType(as3.tolua(LuaRet),Type))
	end
	return false
end
...
function SetWndItemGUIDataPropByKeyName(Value,count,_IsShowBind)
	if not as3.tolua(UI:Lua_GetItemTemplateHandleByKeyName(Value)) then return false end
	local _ItemTemplateHandle = as3.tolua(LuaRet)
	if not as3.tolua(UI:Lua_GetItemTemplatePropByHandle(_ItemTemplateHandle,ITEM_PROP_ID)) then return false end
	if not SetWndItemGUIDataPropByType(ITEMGUIDATA_ITEMID,as3.tolua(LuaRet)) then return false end
	if not as3.tolua(UI:Lua_GetItemTemplatePropByHandle(_ItemTemplateHandle,ITEM_PROP_ICON)) then return false end
	if not SetWndItemGUIDataPropByType(ITEMGUIDATA_IMAGEID,as3.tolua(LuaRet)) then return false end
...
function SetWndItemGUIDataPropByItemID(Value,count,_IsShowBind)
	if not as3.tolua(UI:Lua_GetItemTemplateHandleByID(Value)) then return false end
	local _ItemTemplateHandle = as3.tolua(LuaRet)
...
function RDItemFillGUIData(_Handle, _Info, _GUIDataHandle)
	...
	if as3.tolua(UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ITEMGUID)) then
		if not SetWndItemGUIDataPropByType(ITEMGUIDATA_ITEMGUID, as3.tolua(LuaRet)) then return false end
	end
	...
	if as3.tolua(UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ITEMCOUNT)) then
		if not SetWndItemGUIDataPropByType(ITEMGUIDATA_ITEMCOUNT, as3.tolua(LuaRet)) then return false end
	end
	if as3.tolua(UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_ISSHOWBIND)) then
		if not SetWndItemGUIDataPropByType(ITEMGUIDATA_ISSHOWBIND, as3.tolua(LuaRet)) then return false end
	end
	if as3.tolua(UI:Lua_GetItemGUIDataPropByType(_GUIDataHandle, ITEMGUIDATA_INVALIDATE)) then
		if not SetWndItemGUIDataPropByType(ITEMGUIDATA_INVALIDATE, as3.tolua(LuaRet)) then return false end
	end
	return true
end
...
function CreateWindowFromLayOut(_ID,_LayOutName,_IsCanDrag,_IsEscClose,_ParentHandle)
	...
	if as3.tolua(UI:Lua_FindChildM("GUI_2")) then
...
function CreateCommonWnd(id, width, height, title_image_id)
	...
	if not as3.tolua(UI:Lua_GetWindow("")) then return 0 end
	...
	if title_image_id ~= nil and title_image_id ~= 0 then
		local imgw, imgh
		if as3.tolua(UI:Lua_GetTextureWidthHeight(title_image_id)) then
			imgw = as3.tolua(LuaRet[1]); imgh = as3.tolua(LuaRet[2])
		else
			imgw = 184; imgh = 49
		end
...
function CreateCommonBack(id, handle, info, title, desc_str, desc_height)
	...
	if title ~= nil and title ~= "" and as3.tolua(UI:Lua_GetFontStrWidth("SIMLI18", title)) then
		local text_width = as3.tolua(LuaRet)
...
function MessageBox(flag, info, ok_script, close_script, is_esc_close)
	...
	if nil ~= close_script and "" ~= close_script then
		...
		WndRegistScript(msgbox, nil, RDWndBaseCL_wnd_close, "MessageBoxCloseScript_")
	end
...
function AddMagicToWnd(wnd_id, posx, posy, magic_id, image_id, speed, timer_elapse, timer_func)
	if not as3.tolua(UI:Lua_GetWindow(wnd_id)) then return false end
...
function MoveMagicToBottom(parent_id, magic_id)
	if not as3.tolua(UI:Lua_GetWindow(parent_id)) then return false end
	if not as3.tolua(UI:Lua_FindChildM(magic_id)) then return false end
...
function CreateSimpleCtrl(UIType,ImageID,PosX,PosY,SizeX,SizeY,UIID,AlignType,UIHint)
	...
	_LayStr = _LayStr.."/>"
	if as3.tolua(UI:Lua_WndCreateCtrl(UIType,_LayStr)) then return as3.tolua(LuaRet) end
	return 0
end
...
function Sys_GetItemWindowGUIDList(id)
	...
	CLSetAsRet(as3.toarray(ret))
	return as3.toarray(ret)
end
...
function Sys_ItemWindowMoveIn()
	local id = tonumber(as3.tolua(LuaParam[1]))
	local GUIData = as3.tolua(LuaParam[2])
	if SysItemWindowList[id] == nil then return end
	local t = SysItemWindowList[id].t
	if not as3.tolua(UI:Lua_GetItemGUIDataPropByType(GUIData, ITEMGUIDATA_ITEMGUID)) then return end
	local GUID = as3.tolua(LuaRet)
	if not as3.tolua(UI:Lua_GetItemGUIDataPropByType(GUIData, ITEMGUIDATA_ITEMID)) then return end
	local ItemId = as3.tolua(LuaRet)
	if not as3.tolua(UI:Lua_GetItemTemplatePropByID(ItemId, ITEM_PROP_KEYNAME)) then return end
	local KeyName = as3.tolua(LuaRet)
	local ItemPos = nil
	if GUID ~= 0 and GUID ~= "" then
		if not as3.tolua(UI:Lua_GetItemEntityPropByGUID(GUID, ITEM_PROP_ENTITY_SITE)) then return end
		ItemPos = as3.tolua(LuaRet)
	end
	if not as3.tolua(UI:Lua_GetItemGUIDataPropByType(GUIData, ITEMGUIDATA_ISSHOWBIND)) then return end
	local IsBound = as3.tolua(LuaRet)
	if not as3.tolua(UI:Lua_GetItemGUIDataPropByType(GUIData, ITEMGUIDATA_ITEMCOUNT)) then return end
	local Count = as3.tolua(LuaRet)
	for k, v in pairs(t) do
...
function Sys_ItemCtrlLButtonDown(this)
	...
	if as3.tolua(UI:Lua_GetMouseGUIItemData()) then
		local GUIData = as3.tolua(LuaRet)
		for k, v in pairs(t) do
			if this == v[1] then
