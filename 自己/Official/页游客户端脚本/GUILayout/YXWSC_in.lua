...
local ui = "<form> <dialog OnInit='YXWSC_in.UIInit' id='YXWSC_in' image='0' x='0' y='250' w='16' h='16' revmsg='true'>"
	...
	.."	<richedit id='leaveMap' x='137' y='291' w='134' h='18' text='#SELECT#退出活动#SELECTEND#' text_color='#E4C19F' OnLButtonUp='YXWSC_in.LeaveMap' revmsg='true' font='system'/>"
	.."	<edit id='title' x='70' y='-27' w='92' h='26' revmsg='true' text_color='#F1E761' text='群英争霸' font='SIMLI18'/>"
...
function YXWSC_in.UIInit(h)
	YXWSC_in._hideWnd = 0
	YXWSC_in._wnd = h
	local r1 = "<font color='#9C846D'>1.参与超过5分钟，可领取</font><font color='#00A529'>9个狂暴之灵(小)、15000点BOSS积分、\n10000成就积分</font>\n"
	local r2 = "<font color='#9C846D'>2.被杀一次，可领取</font><font color='#00A529'>3个狂暴之灵(小)、5000点BOSS积分</font>\n"
	local r3 = "<font color='#9C846D'>3.击杀10人，可领取</font><font color='#00A529'>6个狂暴之灵(小)、15000点BOSS积分、10000\n成就积分</font>\n"
	local r4 = "<font color='#9C846D'>4.入前10名，可领取大量</font><font color='#00A529'>狂暴之灵、BOSS积分、成就积分</font><font color='#9C846D'>，排\n名越高奖励越丰厚</font>"
	local handle = 0
	handle = GetWindow(h, "awardHint")
	if handle ~= 0 then
		GUIWndSetHint(handle, "<font color='#D58840'>参与奖励：\n</font>"..r1..r2..r3..r4)
	end
	local s1 = "<font color='#9C846D'>1.玩家随机加入两个对立阵营，每5分钟重新分配阵营</font>\n"
	local s2 = "<font color='#9C846D'>2.击杀玩家、杀怪、持续参与活动都可获得积分</font>\n"
	local s3 = "<font color='#9C846D'>3.活动结束后将根据排名发放奖励</font>\n"
	local handle = 0
	...
	if GUIWndAddTimerBrief(h, 1000, "YXWSC_in.DcUpdata") then    --设置定时器
...
--[[function YXWSC_in.cleartime()  --关闭定时器
