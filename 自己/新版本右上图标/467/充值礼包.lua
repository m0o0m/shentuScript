...
function check_first_bill(player)
	...
	if first_bill_gift[chongzhi_cishu] ~= nil then
        AddSEIcon(player, 1, 0, "充值礼包", 1903800022, "倒计时", "main", "充值礼包", "充值礼包")
		AddIconMagic(player, "充值礼包", 1094200000, -6, -6, 150)
    end
end
...
------------------------领取按钮被点击或关闭--------------------- --我说你妈的为甚么点击按钮会调用这个函数，因为，TimeGiftItemWnd提交表单到充值礼包表单，表单再调用这个函数了啊
------------------------图标点击或关闭--------------------- --但是你妈的，第二次点击图标不是关闭窗口，怎么调用这个函数了啊，什么鬼
function get_first_bill_gift(player)
	...
	lualib:SetDBNumEx(id,lualib:GetDBNum(id)+1,2) --领取次数增加
