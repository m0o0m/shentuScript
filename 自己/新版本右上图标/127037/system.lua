...
---------------------------------------------------------------319
	local FZ_KeyName = {--最大生命值伤害  	，当前生命值伤害
	["符咒1"]	= 3		,
	["符咒2"]	= 6		,
	["符咒3"]	= 9		,
	["符咒4"]	= 12	,
...
function on_pre_harm(role, attacker, hp, defense, skill_key,isBurst)  --被攻击者，攻击者，血量（正加负扣），防御值，技能，暴击
	...
	if lualib:Player_IsPlayer(attacker) then 
		...
		local juju = lualib:JuJu(attacker)
		if juju ~= "" then
			local KeyName = lualib:KeyName(juju);
			-- local mb_rate = lualib:GetInt(attacker, "dizziness");
			local mb_rate = FZ_KeyName[KeyName];	
			if mb_rate ~= 0 and mb_rate ~= nil then 
				...
				-- lualib:SysPromptMsg(attacker, "sub_rate============"..mb_rate);
...
function on_summon(role, monster_id, level)
    print(lualib:Name(role).."召唤了一直"..level.."级的BB[ID:"..monster_id.."]")
end