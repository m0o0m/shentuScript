...
function ForgeWnd5:main()
    local _Parent = LuaGlobal["AttachPartent"]
    local _GUIHandle = 0
    local _DeviceSizeX = CL:GetScreenWidth()
    local _DeviceSizeY = CL:GetScreenHeight()

    _Parent = GUI:WndCreateWnd(_Parent,"ForgeWnd5",0,0,0)
    if _Parent ~= 0 then
    end
    ...
    if _GUIHandle ~= 0 then
        ...
        GUI:EditSetTextM(_GUIHandle,"蕴魂传承")
    ...
    -- 第二层背景
    _GUIHandle = GUI:ImageCreate(_Parent,"BgImage0",0,70,64)
    if _GUIHandle ~= 0 then
         GUI:WndSetSizeM(_GUIHandle,_DeviceSizeX-140, 446)
    end
	
	--第三层背景,装备背景
	_GUIHandle = GUI:ImageCreate(_Parent,"BgImage1",0,100,100)
    if _GUIHandle ~= 0 then
        GUI:WndSetSizeM(_GUIHandle,_DeviceSizeX-200, 320)
    end
    ...
    if _GUIHandle ~= 0 then
    	GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "ForgeWnd5.CloseWnd")
    end
...
--UI Logic Code Start
function ForgeWnd5:UIInit(_GUIHandle)
	-- RegisterUIEvent(LUA_EVENT_PROPERTYDATA, "ForgeWnd5", ForgeWnd5.propDataChange)
	RegisterUIEvent(LUA_EVENT_PLAYERATTREXTREF, "ForgeWnd5", ForgeWnd5.propDataChange)
	...
	UI:Lua_OpenWindow(_GUIHandle,"Inherit.lua")
	local _handle = GetWindow(self._listView, "ItemBgBtn1")
	if _handle ~= 0 then 
		ForgeWnd5._ClickItem(_handle)
	end
end

function ForgeWnd5.CloseWnd()	
	GUI:WndClose(ForgeWnd5.WndHandle);
end

--创建装备列表
function ForgeWnd5:CreateEquipList()
    if UI:Lua_GetPlayerSelfEquipData() then
        if type(LuaRet) ~= "number" then
            self.ItemList = LuaRet
			-- dbg('ForgeWnd5.Equip='.. #self.ItemList)
    ...
    if CL:GetPackageItemGUIDList() then
        self.ItemList = LuaRet
		-- dbg('ForgeWnd5.Package='.. #self.ItemList)
		-- dbg(serialize(self.ItemList))
    end
...
--筛选出身上装备
function ForgeWnd5:item_filter()
...
function ForgeWnd5:CreateUI(i,index)
	...
	if _Parent ~= 0 then
		...
		GUI:WndRegistScript(_Parent,RDWndBaseCL_mouse_lbUp, "ForgeWnd5._ClickItem")
	end
...
--物品框点击回调事件
function ForgeWnd5._ClickItem(_Handle)	
	if ForgeWnd5._saveHanle ~= 0 and ForgeWnd5._saveHanle ~= nil then 
		GUI:ButtonSetIsActivePageBtn(ForgeWnd5._saveHanle, false);
	end
	GUI:ButtonSetIsActivePageBtn(_Handle, true);
	ForgeWnd5._saveHanle = _Handle;
    ...
    if _handle ~= 0 then 
		...
		if item_guid ~= 0 and item_guid ~= nil then
			if UI:Lua_GUID2Str(item_guid) then
				item_guid = LuaRet;
				UI:Lua_SubmitForm("宝石表单", "OfferData", item_guid)
			end
		end
    end
end	
...
function ForgeWnd5.Get_EquipUpdate()
	local handle = GUI:WndFindChildM(ForgeWnd5.WndHandle,"roleequip")	
	local Is_true = ButtonGetIsActivePageBtn(handle)
	for i = 1 , #ForgeWnd5.itemlist do 
			local handle = GUI:WndFindChildM(ForgeWnd5.WndHandle,"itemback"..i)	
			GUI:ItemCtrlClearItemData(handle)
	end
	if Is_true then		
		if UI:Lua_GetPlayerSelfEquipData() then
			ForgeWnd5.itemlist = LuaRet
		end
	else
		if CL:GetPackageItemGUIDList() then
			ForgeWnd5.itemlist = LuaRet
	...
	ForgeWnd5.item_filter()
	ForgeWnd5.forge_data()

end	


function ForgeWnd5.propDataChange()
	
    if LuaParam[2] == ROLE_PROP64_GOLD then
        _GUIHandle = GUI:WndFindChildM(ForgeWnd5.WndHandle, "GoldEdit")
        if _GUIHandle ~= 0 then
            if CL:GetPlayerSelfProperty64(ROLE_PROP64_GOLD) then 
                Property = LuaRet
                GUI:EditSetTextM(_GUIHandle,Property) 
			else
				msg("ForgeWnd5获取玩家64位属性失败")
            end    
        end
    end

    if LuaParam[2] == ROLE_PROP64_INGOT then
        _GUIHandle = GUI:WndFindChildM(ForgeWnd5.WndHandle, "IngotEdit")
        if _GUIHandle ~= 0 then
            if CL:GetPlayerSelfProperty64(ROLE_PROP64_INGOT) then 
                Property = LuaRet
                GUI:EditSetTextM(_GUIHandle,Property)   
