...
function on_create_item(item)
	...
	if item_type == 5 then 
		-- lualib:SysPromptMsg(player, "on_create_item================-----------------")
		if YunHun[item_keyname] then 
			local tab = YunHun[item_keyname];
			for i = 1, #tab[1] do
				lualib:SetDynamicAttr(item, i, tab[1][i], tab[2][i]);
			end
			lualib:SetInt(item, "yunhun_consume", tab[3])
		end
	end
