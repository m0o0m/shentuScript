...
function ForgeWnd2:main()
    local _Parent = LuaGlobal["AttachPartent"]
    local _GUIHandle = 0
    local _DeviceSizeX = CL:GetScreenWidth()
    local _DeviceSizeY = CL:GetScreenHeight()

    _Parent = GUI:WndCreateWnd(_Parent,"ForgeWnd2",0,0,0)
    if _Parent ~= 0 then
    end
    ...
    if _GUIHandle ~= 0 then
        ...
        GUI:EditSetTextM(_GUIHandle,"装备蕴魂")
    ...
    -- if _GUIHandle ~= 0 then
        -- GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "ForgeWnd2.Qiangh")
    -- end
    
    -- _GUIHandle = GUI:ButtonCreate(_Parent,"button2",1851000005,830,4)
    -- if _GUIHandle ~= 0 then
        -- GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "ForgeWnd2.Zhuany")
    -- end

    _GUIHandle = GUI:ButtonCreate(_Parent,"Close",1850000423,_DeviceSizeX - 80, 5)
    if _GUIHandle ~= 0 then
    	GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "ForgeWnd2.CloseWnd")
    end
...
--UI Logic Code Start
function ForgeWnd2:UIInit(_GUIHandle)
	-- RegisterUIEvent(LUA_EVENT_PROPERTYDATA, "ForgeWnd2", ForgeWnd2.propDataChange)
	RegisterUIEvent(LUA_EVENT_PLAYERATTREXTREF, "ForgeWnd2", ForgeWnd2.propDataChange)
	...
	if _handle ~= 0 then 
		ForgeWnd2._ClickItem(_handle)
	end
end

function ForgeWnd2.CloseWnd()	
	GUI:WndClose(ForgeWnd2.WndHandle);
end

--创建装备列表
function ForgeWnd2:CreateEquipList()
    if UI:Lua_GetPlayerSelfEquipData() then
        if type(LuaRet) ~= "number" then
            self.ItemList = LuaRet
			-- dbg('ForgeWnd2.Equip='.. #self.ItemList)
    ...
    if CL:GetPackageItemGUIDList() then
        self.ItemList = LuaRet
		-- dbg('ForgeWnd2.Package='.. #self.ItemList)
		-- dbg(serialize(self.ItemList))
    end
...
--筛选出身上装备
function ForgeWnd2:item_filter()
...
function ForgeWnd2:CreateUI(i,index)
...
--物品框点击回调事件
function ForgeWnd2._ClickItem(_Handle)	
	if ForgeWnd2._saveHanle ~= 0 and ForgeWnd2._saveHanle ~= nil then 
		GUI:ButtonSetIsActivePageBtn(ForgeWnd2._saveHanle, false);
	end
	GUI:ButtonSetIsActivePageBtn(_Handle, true);
	ForgeWnd2._saveHanle = _Handle;
    ...
    if _handle ~= 0 then 
		...
		if item_guid ~= 0 and item_guid ~= nil then
			if UI:Lua_GUID2Str(item_guid) then
				item_guid = LuaRet;
				UI:Lua_SubmitForm("注灵表单", "OfferData", item_guid)
			end
		end
    end
end	
...
function ForgeWnd2.Get_EquipUpdate()
	local handle = GUI:WndFindChildM(ForgeWnd2.WndHandle,"roleequip")	
	local Is_true = ButtonGetIsActivePageBtn(handle)
	for i = 1 , #ForgeWnd2.itemlist do 
			local handle = GUI:WndFindChildM(ForgeWnd2.WndHandle,"itemback"..i)	
			GUI:ItemCtrlClearItemData(handle)
	end
	if Is_true then		
		if UI:Lua_GetPlayerSelfEquipData() then
			ForgeWnd2.itemlist = LuaRet
		end
	else
		if CL:GetPackageItemGUIDList() then
			ForgeWnd2.itemlist = LuaRet
	...
	ForgeWnd2.item_filter()
	ForgeWnd2.forge_data()

end	


function ForgeWnd2.propDataChange()
	
    if LuaParam[2] == ROLE_PROP64_GOLD then
        _GUIHandle = GUI:WndFindChildM(ForgeWnd2.WndHandle, "GoldEdit")
        if _GUIHandle ~= 0 then
            if CL:GetPlayerSelfProperty64(ROLE_PROP64_GOLD) then 
                Property = LuaRet
                GUI:EditSetTextM(_GUIHandle,Property) 
			else
				msg("ForgeWnd2获取玩家64位属性失败")
            end    
        end
    end

    if LuaParam[2] == ROLE_PROP64_INGOT then
        _GUIHandle = GUI:WndFindChildM(ForgeWnd2.WndHandle, "IngotEdit")
        if _GUIHandle ~= 0 then
            if CL:GetPlayerSelfProperty64(ROLE_PROP64_INGOT) then 
                Property = LuaRet
                GUI:EditSetTextM(_GUIHandle,Property)   
