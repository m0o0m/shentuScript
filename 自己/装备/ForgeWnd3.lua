...
function ForgeWnd3:main()
    local _Parent = LuaGlobal["AttachPartent"]
    local _GUIHandle = 0
    local _DeviceSizeX = CL:GetScreenWidth()
    local _DeviceSizeY = CL:GetScreenHeight()

    _Parent = GUI:WndCreateWnd(_Parent,"ForgeWnd3",0,0,0)
    if _Parent ~= 0 then
    end
    ...
    if _GUIHandle ~= 0 then
        ...
        GUI:EditSetTextM(_GUIHandle,"宝石镶嵌")
    ...
    -- if _GUIHandle ~= 0 then
        -- GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "ForgeWnd3.Qiangh")
    -- end
    
    -- _GUIHandle = GUI:ButtonCreate(_Parent,"button2",1851000005,830,4)
    -- if _GUIHandle ~= 0 then
        -- GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "ForgeWnd3.Zhuany")
    -- end

    _GUIHandle = GUI:ButtonCreate(_Parent,"Close",1850000423,_DeviceSizeX - 80, 5)
    if _GUIHandle ~= 0 then
    	GUI:WndRegistScript(_GUIHandle,RDWndBaseCL_mouse_lbUp, "ForgeWnd3.CloseWnd")
    end
...
--UI Logic Code Start
function ForgeWnd3:UIInit(_GUIHandle)
	-- RegisterUIEvent(LUA_EVENT_PROPERTYDATA, "ForgeWnd3", ForgeWnd3.propDataChange)
	RegisterUIEvent(LUA_EVENT_PLAYERATTREXTREF, "ForgeWnd3", ForgeWnd3.propDataChange)
	...
	ForgeWnd3:CreateEquipList()
	UI:Lua_OpenWindow(_GUIHandle,"CrystalWnd.lua")
	local _handle = GetWindow(self._listView, "ItemBgBtn1")
	if _handle ~= 0 then 
		ForgeWnd3._ClickItem(_handle)
	end
end

function ForgeWnd3.CloseWnd()	
	GUI:WndClose(ForgeWnd3.WndHandle);
end

--创建装备列表
function ForgeWnd3:CreateEquipList()
    if UI:Lua_GetPlayerSelfEquipData() then
        if type(LuaRet) ~= "number" then
            self.ItemList = LuaRet
			-- dbg('ForgeWnd3.Equip='.. #self.ItemList)
    ...
    if CL:GetPackageItemGUIDList() then
        self.ItemList = LuaRet
		-- dbg('ForgeWnd3.Package='.. #self.ItemList)
		-- dbg(serialize(self.ItemList))
    end
...
--筛选出身上装备
function ForgeWnd3:item_filter()
...
function ForgeWnd3:CreateUI(i,index)
...
--物品框点击回调事件
function ForgeWnd3._ClickItem(_Handle)	
	if ForgeWnd3._saveHanle ~= 0 and ForgeWnd3._saveHanle ~= nil then 
		GUI:ButtonSetIsActivePageBtn(ForgeWnd3._saveHanle, false);
	end
	GUI:ButtonSetIsActivePageBtn(_Handle, true);
	ForgeWnd3._saveHanle = _Handle;
...
function ForgeWnd3.Get_EquipUpdate()
	local handle = GUI:WndFindChildM(ForgeWnd3.WndHandle,"roleequip")	
	local Is_true = ButtonGetIsActivePageBtn(handle)
	for i = 1 , #ForgeWnd3.itemlist do 
			local handle = GUI:WndFindChildM(ForgeWnd3.WndHandle,"itemback"..i)	
			GUI:ItemCtrlClearItemData(handle)
	end
	if Is_true then		
		if UI:Lua_GetPlayerSelfEquipData() then
			ForgeWnd3.itemlist = LuaRet
		end
	else
		if CL:GetPackageItemGUIDList() then
			ForgeWnd3.itemlist = LuaRet
	...
	ForgeWnd3.item_filter()
	ForgeWnd3.forge_data()

end	


function ForgeWnd3.propDataChange()
	
    if LuaParam[2] == ROLE_PROP64_GOLD then
        _GUIHandle = GUI:WndFindChildM(ForgeWnd3.WndHandle, "GoldEdit")
        if _GUIHandle ~= 0 then
            if CL:GetPlayerSelfProperty64(ROLE_PROP64_GOLD) then 
                Property = LuaRet
                GUI:EditSetTextM(_GUIHandle,Property) 
			else
				msg("ForgeWnd3获取玩家64位属性失败")
            end    
        end
    end

    if LuaParam[2] == ROLE_PROP64_INGOT then
        _GUIHandle = GUI:WndFindChildM(ForgeWnd3.WndHandle, "IngotEdit")
        if _GUIHandle ~= 0 then
            if CL:GetPlayerSelfProperty64(ROLE_PROP64_INGOT) then 
                Property = LuaRet
                GUI:EditSetTextM(_GUIHandle,Property)   
