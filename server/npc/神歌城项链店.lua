function main(npc, player)
	...
	lualib:AddShop(npc, 18)
	local own_family = lualib:GetCastleOwnFamily("神歌城")	
	local msg = ""
	msg = msg.."喔，呵呵~请爽快购买项链。有时我会卖一个很底的价\n"
	msg = msg.."当我粗心时，喔，呵呵~。\n"	
	if own_family ~= "" then
...
function CheckFamily(npc, player)
    ...
    if own_family == "" or own_family ~= my_family then
        ...
        msg = msg.."#OFFSET<X:0,Y:0>##IMAGE1902700033##OFFSET<X:0,Y:-2>#<@OpenShop#8 *01*「确定」>\n \n \n \n"
    else
        msg = "您是城堡的占领者，我可以给您提供打折优惠服务！\n \n \n \n"
        msg = msg.."#OFFSET<X:0,Y:0>##IMAGE1902700033##OFFSET<X:0,Y:-2>#<@OpenShop#18 *01*「确定」>\n \n \n \n"
    end
