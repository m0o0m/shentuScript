local level = 500	--进入副本等级需求

local mapkey = "雪域冰宫一层"		--地图索引名
...
function main(npc, player)
	...
	msg = msg .. "#OFFSET<X:18>##COLORCOLOR_RED#【地图设置】：#COLORCOLOR_GOLD2#《雪域冰宫》隐藏着超级BOSS，终极打宝地图\n"
	msg = msg .. "#OFFSET<X:18>##COLORCOLOR_RED#【进入条件】：#COLORCOLOR_GREENG#500进入\n"
...
function enter(npc, player)
	...
	if lualib:Level(player) < level then
		return "#OFFSET<X:20,Y:35>#抱歉,您的等级不足"..level.."级,无法挑战《雪域冰宫》!"
	end
	
	if lualib:GetIngot(player) < ingot then
		return "#OFFSET<X:20,Y:35>#抱歉,您的元宝不足"..ingot..",无法挑战《雪域冰宫》!"
	end
	
	lualib:SubIngot(player, ingot, "挑战《雪域冰宫》", npc)
